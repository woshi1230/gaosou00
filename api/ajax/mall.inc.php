...
if($job == 'comment') {
	...
	*/
	...
	$pages = pages($items, $page, $pagesize, '#comment" onclick="javascript:load_comment(\'{gaosou_page}&star='.$star.'\');');
	...
	*/
	...
	$pages = pages($items, $page, $pagesize, '#order" onclick="javascript:load_order({gaosou_page});');
	$tmp = explode('<input type="text"', $pages);
	$pages = $tmp[0];
