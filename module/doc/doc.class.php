...
class doc {
    ...
    function isUserDoc($catid) {
		$r = $this->db->get_one("select catid FROM ywb2b.yw_category where moduleid='" . $this->moduleid . "' and catname='用户帮助'");
		$userRootId = $r['catid'];
		$r = $this->db->get_one("select arrparentid FROM ywb2b.yw_category where catid=" . $catid);
		$arrId = $r['arrparentid'];
		if (stristr($arrId, $userRootId)) {
			return true;
		} else {
			return false;
		}
    ...
    function doc($moduleid) {
		global $db, $table, $table_data, $MOD;
		$this->moduleid = $moduleid;
//		$this->table_data = $table_data;
		$this->split = $MOD['split'];
		$this->db = &$db;
		$this->table = $this->db->pre . $table;
		$this->fields = array('catid','areaid','level','title','style','fee','subtitle','introduce','thumb','tag','content','author','copyfrom','fromurl','voteid','status','hits','username','addtime','editor','edittime','ip','template','islink','linkurl','filepath','note');
    }

	function pass($post) {
		if(!is_array($post)) return false;
		if($post['catid'] < 0) return $this->_(lang('message->pass_catid'));
...
//			$r['content'] = $t ? $t['content'] : '';
	...
	function get_list($condition = 'status=3', $order = 'addtime DESC', $cache = '') {
		global $MOD, $pages, $page, $pagesize, $offset, $items, $sum, $types;
		if (!$order) {
			$order = 'addtime DESC';
		}
		if($page > 1 && $sum) {
		...
		if($catids) {
			...
			if($CATS) {
				foreach($lists as $k=>$v) {
					...
					$lists[$k]['caturl'] = $v['catid'] ? $doc_url . '?file=cloud&action=doc&catid=' . $v['catid'] : '';
				}
			}
		}
		return $lists;
	...
	function add($post) {
		global $MOD;
		$post = $this->set($post);
		if ($post['catid'] && $this->isUserDoc($post['catid'])){
			$post['areaid'] = 1;
		} else {
			$post['areaid'] = 0;
		}
