...
if($itemid) {
	...
	if($pass) {
		...
		if($view) {
			...
			$demo_url = userurl($username, 'file='.$file.'&itemid='.$itemid.'&view=1&page={gaosou_page}', $domain).'#p';
			...
			$demo_url = userurl($username, "file=$file&itemid=$itemid&page=".'{gaosou_page}', $domain);
