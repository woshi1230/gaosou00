...
//更改公司主页转盘图片新增
$HOME_Pic= array();
$result = $db->query("SELECT * FROM `{$DT_PRE}photo_12` WHERE `username`='$username'and items>0 ORDER BY `items` DESC LIMIT 0,10");
...
if($_username && ($_username !=$homepage)){
	$r = $db->get_one("SELECT id FROM {$DT_PRE}visit WHERE (username='$_username' AND homepage ='$homepage')");

	if(!$r){
		$db->query("insert {$DT_PRE}visit (username,userid,homepage) values ('$_username','$_userid','$homepage')");
	}
}

//查出所有在线的
$result1 = $db->query("SELECT username FROM {$DT_PRE}online");
...
$result = $db->query("select v.username,c.*,m.truename from {$DT_PRE}visit as v JOIN {$DT_PRE}company as c JOIN {$DT_PRE}member as m on v.username = c.username AND m.username = c.username WHERE v.homepage = '$homepage'");
while ($all = $db->fetch_array($result)) {
	if(in_array($all['username'],$lists2)){
//        $r['online']=1;
		$all['online']=1;
	}else{
		$all['online']=0;
	...
	if(!$all['avatarpic']){
		$all['avatarpic'] ='file/upload/default.jpg';
	}
	$all['avatarpic'] = DT_PATH.$all['avatarpic'];
	if ($all['buyer_level']) {
		$all['buyer_level'] =DT_PATH.substr(strstr($all['buyer_level'], '<'), 10, 35);
	}
...
//
////查出聊天时间
//$resultChat =   $db->query("select ");
//while ($all2Chat = $db->fetch_array($resultChat)) {
//	$friendChat[] = $all2Chat;
//}

$new = 0;
//查出所有聊过天的好友
//$result2 =   $db->query("select f.username,f.itemid,c.*,m.truename,chat.lasttime,chat.tnew  from {$DT_PRE}friend as f inner JOIN {$DT_PRE}company as c INNER JOIN {$DT_PRE}member as m right join  {$DT_PRE}chat as chat     on f.username = c.username AND c.username= m.username AND chat.touser = c.username  WHERE f.userid = '$_userid' AND c.userid !='$_userid' GROUP BY f.itemid");
$result2 =   $db->query("select f.username,f.itemid,c.avatarpic,c.buyer_level,f.truename,chat.* from  yw_company  as c   join yw_friend   as f    JOIN yw_chat as chat on  c.username = f.username and (f.username = chat.fromuser or f.username = chat.touser )  WHERE f.userid = '$_userid' and  f.username != '' and f.agree =1 and (chat.fromuser='$_username' OR chat.touser='$_username') GROUP BY username");
while ($all2 = $db->fetch_array($result2)) {

	if($all2['fromuser'] == $_username) {
		$all2['user'] = $all2['touser'];
		$all2['new'] = $all2['fnew'];
	} else {
		$all2['user'] = $all2['fromuser'];
		$all2['new'] = $all2['tnew'];
	}
	$new += $all2['new'];


	$all2['lasttime']=date('m-d',$all2['lasttime']);
	if(in_array($all2['username'],$lists2)){
//        $r['online']=1;
		$all2['online']=1;
	}else{
		$all2['online']=0;
	...
	if(!$all2['avatarpic']){
		$all2['avatarpic'] ='file/upload/default.jpg';
	}
	$all2['avatarpic'] = DT_PATH.$all2['avatarpic'];
	if ($all2['buyer_level']) {
		$all2['buyer_level'] =DT_PATH.substr(strstr($all2['buyer_level'], '<'), 10, 35);
	}
	$goodFriends[]=$all2['username'];
	$friend[] = $all2;
...
$totalInfo = $db->get_one("select sum(tnew) as total from {$DT_PRE}chat where touser = '$_username'");


//查出同学校同公司
$result4 = $db->query("select school,company from {$DT_PRE}member  WHERE username='$homepage' ");
while ($all4 = $db->fetch_array($result4)) {
	$school = $all4['school'];
	$company = $all4['company'];
}

//$result3 = $db->query("select * from {$DT_PRE}member as m  WHERE (school like  '%$school%' or company LIKE '%$company%')");
$result3 = $db->query("select c.*,m.* from {$DT_PRE}member as m JOIN {$DT_PRE}company as c on m.username = c.username WHERE  (m.school like  '%$school%' or m.company LIKE '%$company%') and m.username!='$homepage' AND m.username NOT IN ( '" . implode($goodFriends, "', '") . "' )");
while ($all3 = $db->fetch_array($result3)) {

	if(in_array($all3['username'],$lists2)){
//        $r['online']=1;
		$all3['online']=1;
	}else{
		$all3['online']=0;
	...
	if(!$all3['avatarpic']){
		$all3['avatarpic'] ='file/upload/default.jpg';
	}
	$all3['avatarpic'] = DT_PATH.$all3['avatarpic'];
	if ($all3['buyer_level']) {
		$all3['buyer_level'] =DT_PATH.substr(strstr($all3['buyer_level'], '<'), 10, 35);
	}
	$know[] = $all3;
}
