...
class user_mark_class {
	...
	public static function get_mark_config($type = null, $obj = null) {
		...
		$sql = " select * from " . TB_PRE . "witkey_mark_config  $where ";
//		$type && $obj and $mark_conf = db_factory::get_one ( $sql ) or $mark_conf = db_factory::query ( $sql );
		$type && $obj and $mark_conf = Yw_witkey_mark_config_class::get_one ( $sql, $type, $obj ) or $mark_conf = yw_witkey_mark_config_class::query ( $sql );
		;
		return $mark_conf;
	}
	...
	public static function mark_info_exists($model_code,$obj_id,$by_uid,$uid){
		return db_factory::execute(sprintf(" select model_code from %switkey_mark where model_code='%s' and obj_id = '%d' and uid='%d' and by_uid ='%d'",TB_PRE,$model_code,$obj_id,$uid,$by_uid));
	}
	...
	public static function get_single_mark($mark_id){
		return db_factory::get_one(sprintf(" select * from %switkey_mark where mark_id = '%d'",TB_PRE,$mark_id));
	}
	public static function exec_rate($mark_status,$uid, $mark_cash, $model_code, $mark_type = '2') {
		...
		$credit_value = db_factory::get_count(sprintf("select %s from yw_company where userid = '%d'",$edit_col,$uid));
		...
		$wheresqlarr = array("userid"=>$uid);
		keke_table_class::updateself("yw_company", $space_array, $wheresqlarr);
	}
...
//		$score_rule = kekezu::get_table_data ( "*", TB_PRE . "witkey_mark_rule", "", "$credit_name asc ", '', '', '', null );
		$score_rule = array ();
		$result = $db->query( " select * from " . TB_PRE . "witkey_mark_rule order by $credit_name asc", $CFG['db_expires']);
	...
	public static function get_mark_rule($type = null) {
		$score_rule = kekezu::get_table_data ( "*", TB_PRE . "witkey_mark_rule", "", "g_value asc,m_value asc ", '', '', '', "3600" );
		$size = sizeof ( $score_rule );
		$score ['wiki'] = array (); 
		$score ['ploy'] = array (); 
		$size = sizeof ( $score_rule );
	...
	public static function get_mark_info($w=array(),$p=array(),$order=null,$ext=null) {
		global $kekezu;
		$where = " select * from ".TB_PRE."witkey_mark where 1 = 1";
	...
	public static function get_user_name($uid) {
		return db_factory::get_count ( " select username from yw_member where userid ='$uid' " );
	}
	...
	public static function get_user_aid($uid, $mark_type, $mark_status = null, $role_type = null, $model_code = null, $obj_id = null) {
		...
		$aid_arr = array ();
		$result = $db->query( " select aid,aid_star from " . TB_PRE . "witkey_mark where $where", $CFG['db_expires']);
		while($r = $db->fetch_array($result)) {
			$aid_arr[] = $r;
		}
...
//		$shop_open and $sql.=',c.title';
		$sql .= ',d.seller_level from '.TB_PRE.'witkey_mark a ';
//		$task_open and $sql.=' left join '.TB_PRE.'witkey_task b on a.origin_id = b.task_id ';;
//		$shop_open and $sql.=' left join '.TB_PRE.'witkey_service c on a.origin_id = c.service_id';
		$sql.=' left join yw_company d on a.by_uid = d.userid';
		$sql.=' where '.$where;
...
//		$aid_config = kekezu::get_table_data('aid,aid_name,aid_type',TB_PRE . 'witkey_mark_aid','','','','','aid');
		$aid_config = Yw_witkey_mark_aid_class::get_table_data('aid,aid_name,aid_type',TB_PRE . 'witkey_mark_aid','','','','','aid');
...
//		$aid_arr = kekezu::get_table_data ( "*", TB_PRE . "witkey_mark_aid" );

		$aid_arr = Yw_witkey_mark_aid_class::get_table_data ( "*", TB_PRE . "witkey_mark_aid" );
		$aid = array ();
	...
	public static function gen_star2($num, $len=5) {
		...
		$str = '<span style="width: 120px;text-align: left;"  class="stars ';
		$str.= 'a'.$len;
		$str.= ' s'.$arr[0];
