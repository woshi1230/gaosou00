...
if($action == 'update') {
	...
	switch($step) {
		...
		case 'pay'://买家付款
			...
			if($submit) {
				...
				//更新商品数据
				if($td['mid'] == 16) {
					$db->query("UPDATE {$DT_PRE}mall SET orders=orders+1,sales=sales+$td[number],amount=amount-$td[number]*(CASE deliver_type WHEN 2 THEN 0 ELSE 1 END) WHERE itemid=$mallid");
				} else {
					$db->query("UPDATE ".get_table($td['mid'])." SET amount=amount-$td[number]*(CASE deliver_type WHEN 2 THEN 0 ELSE 1 END) WHERE itemid=$mallid");
				}
	 ...
	 */
	if($itemid){
		$r = $db->get_one("SELECT a.seller, b.filepath FROM {$table} a, {$DT_PRE}mall b WHERE a.itemid=$itemid and a.buyer='$_username' and (a.status='3' or a.status='4') and a.mallid=b.itemid");
		if ($r) {
			$filepath = $r['filepath'];
			$seller = $r['seller'];
			$r2 = $db->get_one("SELECT userid FROM {$DT_PRE}member WHERE username='$seller'");
			if ($r2) {
				$upload_table = $DT_PRE . 'upload_' . ($r2['userid'] % 10);
				// 上传文件必须是用户（卖家），不能是管理员，否则找不到
				$r3 = $db->get_one("SELECT filename FROM {$upload_table} WHERE fileurl='$filepath'");
				if ($r3) {
					$arrFileLists[] = array("filename"=>$r3['filename'], "filepath"=>$filepath);
					include template('downserverfile', $module);
					die;
				}
			}
		}
	}
	echo "<div style='padding: 20px;'>查无下载文件.</div>";die;
} else {
