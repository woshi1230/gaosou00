...
switch($action) {
	...
	break;
	default:
		$sfields = array('按条件', '金额', '银行', '事由', '备注', '操作人');
		$dfields = array('username', 'amount', 'bank', 'reason', 'note', 'editor');
		...
		while($r = $db->fetch_array($result)) {
			...
			$r['site_profit'] > 0 ? $income += $r['site_profit'] : $expense += $r['site_profit'];
			$records[] = $r;
		}
		...
		include tpl('site_profit', $module);
	break;
}
?>