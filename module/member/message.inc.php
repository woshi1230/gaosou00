...
switch($action) {
	case 'queren':
		$info = userinfo($touser);
		$infoid= $info['userid'];
		$addtime = $DT_TIME;
		$chatid = md5($addtime);
		$res0 = $db->query("INSERT INTO {$DT_PRE}chat (chatid,fromuser,touser,lasttime,lastmsg,forward) VALUES ('$chatid','$_username','$touser','$addtime','d','asd');");
		$res1 = $db->query("UPDATE {$DT_PRE}friend SET agree = 1 WHERE (userid = '$_userid' AND username = '$touser') or (userid = '$infoid' AND username = '$_username')");
		$res2 = $db->query("INSERT INTO {$DT_PRE}message (`title`,`typeid`,`content`,`fromuser`,`touser`,`addtime`,`status`)  VALUES
 									('{$_username}成功添加您为好友','4','{$_username} 先生 已成功添加您为好友，想要在线及时联系他（她），请进入“我的好友-全部好友”列表中查找：','$_username','$touser','$addtime','3')");
		$res3 = $db->query("update {$DT_PRE}member set message = message+1 where username = '$touser'");
