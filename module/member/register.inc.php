...
// 取得用户的初始等级
function getLevel($user_type) {
	require_once DT_ROOT.'/module/company/user_mark_class.php';
	$level = user_mark_class::get_mark_level ( 0, $user_type );
...
if($submit) {
	...
	// 初始化卖家的等级
	$post['seller_level'] = getLevel('1');
	$post['buyer_level'] = getLevel('2');
	if($do->add($post)) {
		$userid = $do->userid;
