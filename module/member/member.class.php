...
class member {
	...
	function is_password($password, $cpassword) {
		...
		if(preg_match($pattern_num,$password) || preg_match($pattern_str,$password)) return $this->_($L['member_password_num']);
		return true;
	}
	...
	function is_member($member) {
		...
		if($this->userid) {
			...
			if($member['groupid'] > 5) {
				...
				if(word_count($member['content'])>0 && word_count($member['content']) < 5) return $this->_($L['member_introduce_null']);//(word_count($member['content'])>0加的，公司介绍，数据表中为空时（data字段），报错
	...
	function set_member($member) {
		...
		if(!defined('DT_ADMIN')) {
			...
			$seller_level = $member['seller_level'];
			$buyer_level = $member['buyer_level'];
			unset($member['content']);
			unset($member['seller_level']);
			unset($member['buyer_level']);
			$member = dhtmlspecialchars($member);
			$member['content'] = dsafe($content);
			$member['seller_level'] = $seller_level;
			$member['buyer_level'] = $buyer_level;
		}
	...
	function add($member) {
		...
		$member['sound'] = 1;
		$member_fields = array('username','school','company','passport', 'password','payword','email','sound','gender','truename','mobile','msn','qq','ali','skype','department','career','groupid','regid','areaid','edittime','inviter','passsalt', 'paysalt');
		$company_fields = array('username','groupid','company','type','catid','catids','areaid', 'mode','capital','regunit','size','regyear','sell','buy','seller_level','buyer_level','business','telephone','fax','mail','address','postcode','homepage','introduce','thumb','keyword','linkurl');
	...
	function edit($member)	{
		...
		$member = $this->set_member($member);
		$r = $this->get_one();
		$member_fields = array('company','passport','sound','email','msn','qq','ali','skype','gender','truename','mobile','department','career','school','groupid','areaid', 'edittime','black','bank','banktype','branch','account','vemail','vmobile','vbank','vtruename','vcompany','vtrade','trade','support','inviter');
