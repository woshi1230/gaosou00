...
<td>
	...
	<tr align="center" class="c_p">
		<td><span onclick="Dalbum('',<?php echo $moduleid;?>,<?php echo '150';?>,<?php echo '110';?>, Dd('thumb').value, true);" class="jt"><img src="<?php echo $MODULE[2]['linkurl'];?>image/img_upload.gif" width="12" height="12" title="上传"/></span>&nbsp;&nbsp;<img src="<?php echo $MODULE[2]['linkurl'];?>image/img_select.gif" width="12" height="12" title="选择" onclick="selAlbum('');"/>&nbsp;&nbsp;<span onclick="delAlbum('', 'wait');" class="jt"><img src="<?php echo $MODULE[2]['linkurl'];?>image/img_delete.gif" width="12" height="12" title="删除"/></span></td>
		<td><span onclick="Dalbum(1,<?php echo $moduleid;?>,<?php echo '150';?>,<?php echo '110'?>, Dd('thumb1').value, true);" class="jt"><img src="<?php echo $MODULE[2]['linkurl'];?>image/img_upload.gif" width="12" height="12" title="上传"/></span>&nbsp;&nbsp;<img src="<?php echo $MODULE[2]['linkurl'];?>image/img_select.gif" width="12" height="12" title="选择" onclick="selAlbum(1);"/>&nbsp;&nbsp;<span onclick="delAlbum(1, 'wait');" class="jt"><img src="<?php echo $MODULE[2]['linkurl'];?>image/img_delete.gif" width="12" height="12" title="删除"/></span></td>
		<td><span onclick="Dalbum(2,<?php echo $moduleid;?>,<?php echo '150';?>,<?php echo '110'?>, Dd('thumb2').value, true);" class="jt"><img src="<?php echo $MODULE[2]['linkurl'];?>image/img_upload.gif" width="12" height="12" title="上传"/></span>&nbsp;&nbsp;<img src="<?php echo $MODULE[2]['linkurl'];?>image/img_select.gif" width="12" height="12" title="选择" onclick="selAlbum(2);"/>&nbsp;&nbsp;<span onclick="delAlbum(2, 'wait');" class="jt"><img src="<?php echo $MODULE[2]['linkurl'];?>image/img_delete.gif" width="12" height="12" title="删除"/></span></td>
	</tr>
	</table>
...
<tr>
	<td class="tl"><span class="f_red">*</span>交付类型</td>
	<td>
		<input type="radio" name="post[deliver_type]" value="1" <?php if($deliver_type=='1') { ?> checked<?php } ?> onclick="DTClick(1);"/> 实物&nbsp;&nbsp;&nbsp;&nbsp;
		<input type="radio" name="post[deliver_type]" value="2" <?php if($deliver_type=='2') { ?> checked<?php } ?> onclick="DTClick(2);"/> 软件&nbsp;&nbsp;&nbsp;&nbsp;
		<span id="ddeliver_type" class="f_red"></span>
	</td>
</tr>

<tr style="display:<?php if($deliver_type=='1') { ?> <?php } else { ?> none <?php } ?>" id="tr_fee">
...
<span class="f_gray">&nbsp;填写示例：<span class="c_p" title="点击观看" onclick="Nexpress('0.00', '包邮');">包邮</span> / <span class="c_p" title="点击观看" onclick="Nexpress('500.00', '包邮');">满500包邮</span> / <span class="c_p" title="点击观看" onclick="Nexpress('10.00', '快递');">快递10元</span> / <span class="c_p" title="点击观看" onclick="Nexpress('500.00', '包邮');Dd('express_name_2').value = '快递';Dd('fee_start_2').value = '10.00';">快递10元，满500包邮</span></span>
	...
	<?php if(false && $MOD['swfu']) { ?>
		<tr style="display:<?php if($deliver_type=='2') { ?> <?php } else { ?> none <?php } ?>
			" id="tr_filepath">
			<td class="tl">作品源件</td>
			<td>
				<div style="float:left;"><input name="post[filepath]" id="filepath" type="text" size="60" value="<?php echo $fileurl;?>"/>&nbsp;&nbsp;</div><div style="width:34px;height:20px;float:left;"><span id="spanButtonPlaceHolder"></span></div><table cellspacing="0" style="display:none;">
					<tr>
						<td>Files Queued:</td>
				...
				</table><div style="float:left;">&nbsp;&nbsp;<span onclick="if(Dd('filepath').value) window.open(Dd('filepath').value);" class="jt">[预览]</span>&nbsp;&nbsp;<span onclick="Dd('filepath').value='';" class="jt">[删除]</span>&nbsp;&nbsp;
					<span class="f_gray">进度：<span id="tdPercentUploaded">0%</span></span> <span id="dfilepath" class="f_red"></span></div>
...
// The event handler functions are defined in handlers.js
	...
	<?php } else { ?>
		<tr style="display:<?php if($deliver_type=='2') { ?> <?php } else { ?> none <?php } ?>
			" id="tr_filepath">
			<td class="tl"><span class="f_hid">*</span> 作品源件</td>
			<td><input name="post[filepath]" id="filepath" type="text" size="60" value="<?php echo $filepath;?>"/>&nbsp;&nbsp;<span onclick="Dfile(<?php echo $moduleid;?>, Dd('filepath').value, 'filepath', 'rar|zip|pdf|jpg|gif|png|doc|ppt|xls|docx|pptx|xlsx');" class="jt">[上传]</span>&nbsp;&nbsp;<span onclick="if(Dd('filepath').value) window.open(DTPath + Dd('filepath').value);" class="jt">[下载]</span>&nbsp;&nbsp;<span onclick="Dd('filepath').value='';" class="jt">[删除]</span></td>
		</tr>
	<?php } ?>
...
<td><input type="text" size="50" name="post[filepath]" value="<?php echo $filepath;?>" id="filepath"/>&nbsp;<input type="button" value="重名检测" onclick="ckpath(<?php echo $moduleid;?>, <?php echo $itemid;?>);" class="btn"/>&nbsp;<?php tips('可以包含目录和文件 例如 gaosou/b2b.html<br/>请确保目录和文件名合法且可写入，否则可能生成失败');?>&nbsp; <span id="dfilepath" class="f_red"></span></td>
</tr>
<?php } ?>
</table>
...
function DTClick(val) {
	if (val == 1) {
		Dd('tr_fee').style.display='';
		Dd('tr_filepath').style.display='none';
		$("input[name='post[deliver_type]'][value='1']").attr("checked",true);
		$("input[name='post[deliver_type]'][value='2']").attr("checked",false);
	} else {
		Dd('tr_fee').style.display='none';
		Dd('tr_filepath').style.display='';
		$("input[name='post[deliver_type]'][value='1']").attr("checked",false);
		$("input[name='post[deliver_type]'][value='2']").attr("checked",true);
	}
}
function check() {
	var l;
	...
	f = 'post[deliver_type]';
	l = $("input[name='post[deliver_type]'][checked]").val();
	if(!l) {
		Dmsg('请选择交付类型', 'deliver_type');
		return false;
	}
	f = 'content';
	l = FCKLen();
