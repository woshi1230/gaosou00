...
class mall {
    ...
    function mall($moduleid) {
		...
		$this->db = &$db;
		$this->fields = array('catid','mycatid','areaid','level','title','style','fee','introduce','brand','price','step','amount','unit','thumb','thumb1','thumb2','tag','status','hits','username','editor','addtime','adddate','edittime','editdate','ip','template','linkurl','filepath','elite','note','company','truename','telephone','mobile','address','email','msn','qq','ali','skype','n1','n2','n3','v1','v2','v3','express_1','express_name_1','fee_start_1','fee_step_1','express_2','express_name_2','fee_start_2','fee_step_2','express_3','express_name_3','fee_start_3','fee_step_3','cod','deliver_type');
    }
	...
	function pass($post) {
		...
		if(!is_uri($post['thumb'])) return $this->_(lang('message->pass_thumb'));
	...
	function set($post) {
		...
		is_uri($post['thumb']) or $post['thumb'] = '';
		is_uri($post['thumb1']) or $post['thumb1'] = '';
		is_uri($post['thumb2']) or $post['thumb2'] = '';
//		$post['filepath'] = (isset($post['filepath']) && is_filepath($post['filepath'])) ? file_vname($post['filepath']) : '';
		...
		if($this->itemid) {
			...
			delete_diff($new, $old);
			if ($post['filepath'] != $r['filepath']) {
				delete_upload($r['filepath'], $_userid);
			}
		} else {
			$post['ip'] = $DT_IP;
		}
	...
	function add($post) {
		...
		clear_upload($post['content'].$post['thumb'].$post['thumb1'].$post['thumb2'].$post['filepath'], $this->itemid);
		return $this->itemid;
	}
	...
	function delete($itemid, $all = true) {
		...
		if(is_array($itemid)) {
			...
			if($all) {
				...
				if($r['filepath']) delete_upload($r['filepath'], $userid);
