	...
	if($post) {
		...
		foreach($post as $k=>$v) {
			...
			if($t && $t['status'] == 3 && $t['username'] != $_username && $t['amount'] > 0) {
				...
				$deliver_type = $t['deliver_type'];

				$db->query("INSERT INTO {$DT_PRE}mall_order (mid,mallid,buyer,seller,title,thumb,price,number,amount,addtime,updatetime,note, buyer_postcode,buyer_address,buyer_name,buyer_phone,buyer_mobile,status,fee_name,fee,cod,deliver_type) VALUES ('$moduleid','$itemid','$_username','$t[username]','$title','$t[thumb]','$price','$number','$amount','$DT_TIME','$DT_TIME','$note','$buyer_postcode','$buyer_address','$buyer_name','$buyer_phone','$buyer_mobile','$status','$fee_name','$fee','$cod','$deliver_type')");
				$oid = $db->insert_id();
