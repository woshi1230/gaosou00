...
switch($action) {
	...
	case 'show':
		...
		if(!isset($fid)) {
			$r = $db->get_one("SELECT min(task_id) AS fid FROM {$table} WHERE task_status>2 {$sql}");
			$fid = $r['fid'] ? $r['fid'] : 0;
		}
		isset($sid) or $sid = $fid;
		if(!isset($tid)) {
			$r = $db->get_one("SELECT max(task_id) AS tid FROM {$table} WHERE task_status>2 {$sql}");
			$tid = $r['tid'] ? $r['tid'] : 0;
		}
		if($update) {
		...
		if($fid <= $tid) {
			$result = $db->query("SELECT task_id FROM {$table} WHERE task_status>2 AND task_id>=$fid {$sql} ORDER BY task_id LIMIT 0,$num ");
	...
	break;
	default:
//		注：本页面中做数据库查询时，itemid改为task_id，status改为task_status，其中itemid，status为以前表字段
//		$r = $db->get_one("SELECT min(itemid) AS fid,max(itemid) AS tid FROM {$table} WHERE status=3");task_status
		$r = $db->get_one("SELECT min(task_id) AS fid,max(task_id) AS tid FROM {$table}");
		$fid = $r['fid'] ? $r['fid'] : 0;
		$tid = $r['tid'] ? $r['tid'] : 0;
