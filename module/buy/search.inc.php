...
//		if($keyword) $condition .= " AND $dfields[$fields] LIKE '%$keyword%'";// $dfields为 0 => 'keyword',1 => 'title', 3 => 'introduce',wikey_task表中无此字段
		if($keyword) $condition .=' AND '."  task_title LIKE '%$keyword%' ";
		if($pptsql) $condition .='AND'.$pptsql;//PPT
		$condition = " task_status>1 ".$condition;
	//	$condition = "status=3".$condition;wikey_task表中无status字段
	}
	...
	if($items) {
		...
		$result = $db->query("SELECT task_id,task_title,task_cash FROM {$table} WHERE {$condition} ORDER BY start_time desc LIMIT {$offset},{$pagesize}", $DT['cache_search'] && $page == 1 ? 'CACHE' : '', $DT['cache_search']);
		if($kw) {
			$replacef = explode(' ', $kw);
