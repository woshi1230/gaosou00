...
require_once DT_ROOT.'/include/session_'.$CFG['session'].'.class.php';
...
$_groupid = 3;
$gaosou_auth = get_cookie('auth');
if($gaosou_auth) $gaosou_auth = decrypt($gaosou_auth, DT_KEY.'USER');
if($gaosou_auth) {
	$_dauth = explode('|', $gaosou_auth);
	...
	if($_userid) {
		...
		unset($gaosou_auth, $USER, $_dauth, $_password);
	}
}
