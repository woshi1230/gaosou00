...
if (!in_array($from, array('thumb', 'album', 'photo', 'gallery', 'editor', 'attach', 'file'))) {
...
$js = $errjs = '';
if ($from == 'thumb' || $from == 'album' || $from == 'photo' || $from == 'gallery' || $from == 'file') {
    $errjs .= 'window.parent.cDialog();';
} else if ($from == 'editor' || $from == 'attach') {
    $errjs .= 'window.parent.GetE("frmUpload").reset();';
}

// 当在图片管理中新增图片时，计算是否超过当前用户允许的最大图库空间设置
if ($from == 'gallery' && $MG['uploadtotalsize']) {
//	$r = $db->get_one("SELECT sum(filesize) AS totalsize FROM {$upload_table} WHERE moduleid='2' and username='$_username'");
    if ($_groupid == 1 || $_groupid == 7) {
    } else {
        $r = $db->get_one("SELECT sum(filesize) AS totalsize FROM {$upload_table} WHERE upfrom='gallery' and username='$_username'");
        if ($r['totalsize'] >= $MG['uploadtotalsize'] * 1024 * 1024) {
            $errmsg = '空间不够！';
            if ($swfupload) exit(convert($errmsg, DT_CHARSET, 'utf-8'));
            dalert($errmsg);
        }
...
if ($do->save()) {
    ...
    if ($do->image) {
        ...
        if ($do->ext == 'gif' && in_array($from, array('thumb', 'album', 'photo', 'gallery'))) {
...
//	$saveto = linkurl($do->saveto);
    ...
    if (isset($old) && $old && in_array($from, array('thumb', 'photo', 'gallery'))) delete_upload($old, $_userid);
    $_saveto = $swfupload ? str_replace('.thumb.' . $do->ext, '', $saveto) : $saveto;
    if ($from != 'gallery') {
        $_SESSION['uploads'][] = $_saveto;
    }
    if ($DT['uploadlog']) {
        $db->query("INSERT INTO {$upload_table} (item,fileurl,filename,filesize,fileext,upfrom,width,height,moduleid,username,ip,addtime,itemid) VALUES ('" . md5($saveto) . "','$saveto','$do->file_name','$do->file_size','$do->ext','$from','$img_w','$img_h','$moduleid','$_username','$DT_IP','$do->uptime','$itemid')");
...
//		$js .= 'window.parent.getAlbum("'.$saveto.'", "'.$fid.'");';
        $js .= $pr . '("user_gallery").action=\'?action=index&editor=' . $editor . '&catid=' . $fid . '&uploadid=' . $uploadid . '\';';
        $js .= $pr . '("user_gallery").submit();';
    } else if ($from == 'editor') {
