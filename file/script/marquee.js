function dmarquee(lineheight, speed, delay, id) {
	...
	this.scrolling = function() {
		if(this.o.scrollTop%lineheight !=0) {
			...
			var tmpv = this.o.scrollTop;
			this.o.scrollTop = this.o.scrollTop + 1;
			if (tmpv === this.o.scrollTop) {
				this.o.scrollTop = 0;
			}
			clearInterval(this.t); setTimeout(function() {_this.start();}, delay);
		}
	}
...
function dmarquee2(lineheight, speed, delay, id) {
	this.t; this.p = false; this.o = Dd(id); this.h = this.o.innerHTML; if(this.h.length < 10) return;
	this.o.innerHTML = '<div id="'+id+'_tmp"><div>'+this.h+'</div></div>';
	var h1 = Number(Dd(id).style.height.replace('px', '')); var h2 = Dd(id+'_tmp').scrollHeight;
	if(lineheight == -1) return;
	...
	this.start = function() {
		...
		this.t = setInterval(function() {_this.scrolling();}, speed);
	...
	this.scrolling = function() {
		if(this.o.scrollTop%lineheight !=0) {
			...
			this.timer = setInterval(function() {_this.start();}, delay);
		}
	}
	this.timer = setInterval(function() {_this.start();}, delay);
}
