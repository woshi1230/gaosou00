		...
		scroller.fn.goto=function(d){
			...
			case "bottom":
				...
				if(d=="right"){
					...
					// console.log('设置背景色');
					var seri = parseInt(_max / _dis) - 1;
					if (seri < 0) seri = 0;
					var bkc = scroller.items[seri].style.background;
					scroller.box.parent().css({"background": bkc});
					//scroller.items.animate({"opacity":0},settings.speed,function(){
					//	scroller.items[seri].style.opacity = 1;
					//});

					if(_dis>_max){_dis=_max};
					scroller.box.animate({"scrollTop":_dis},settings.speed,function(){
						if(parseInt(scroller.box.scrollTop(),10)>=_max || seri == 0){
							scroller.box.scrollTop(0);
						};
					});
				};
