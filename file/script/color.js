...
color_htm += '<td title="gaosou Color Selector Powered By gaosou.COM">&nbsp;&nbsp;&nbsp;</td>';
color_htm += '<td align="right" style="color:#FFFFFF;font-weight:bold;cursor:pointer;" onclick="color_close();" title="Close">&#215;&nbsp;</td>';
color_htm += '</tr>';
color_htm += '</table>';
color_htm += '<div id="gaosou_color_show"></div>';
function color_show(id, color, obj) {
	Eh();
	if(Dd('gaosou_color') == null) {
		var gaosou_color_div = document.createElement("div");
		with(gaosou_color_div.style) {zIndex = 9999; position = 'absolute'; display = 'none'; width = '160px'; padding = '1px'; top = 0; left = 0; border = '#A0A0A0 1px solid'; backgroundColor = '#FFFFFF';}
		gaosou_color_div.id = 'gaosou_color';
		document.body.appendChild(gaosou_color_div);
	}
	var aTag = obj; var leftpos = toppos = 0;
	do {aTag = aTag.offsetParent; leftpos	+= aTag.offsetLeft; toppos += aTag.offsetTop;
	} while(aTag.offsetParent != null);
	Dd('gaosou_color').style.left = (obj.offsetLeft + leftpos) + 'px';
	Dd('gaosou_color').style.top = (obj.offsetTop + toppos + 20) + 'px';
	Dd('gaosou_color').innerHTML = color_htm;
	color_id = id;
	color_bk = color;
	Dd('gaosou_color').style.display = '';
	if(color) color_view(color);
	color_setup(color);
}
function color_hide() {Dh('gaosou_color'); Es();}
...
function color_setup(color) {
	...
	colors_select += '</table>';
	Dd('gaosou_color_show').innerHTML = colors_select;
}
