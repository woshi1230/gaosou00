...
function Dgallery(f, m, w, h, o, s) {
	var s = s ? 'none' : '';
	var c = '<iframe name="UploadGallery" style="display:none" src=""></iframe>';
	c += '<form method="post" target="UploadGallery" enctype="multipart/form-data" action="'+UPPath+'" ' +
		'onsubmit="return isUP(\'upgallery\');">' +
		'<input type="hidden" name="fid" value="'+f+'"/>' +
		'<input type="hidden" name="moduleid" value="'+m+'"/>' +
		'<input type="hidden" name="from" value="gallery"/>' +
		'<input type="hidden" name="old" value="'+o+'"/>' +
		'<table cellpadding="3"><tr><td><input id="remote_0" type="radio" name="isremote" value="0" checked' +
		' onclick="ReLo(0, \'upgallery\');"/><label for="remote_0"> '+L['up_local']+'</label>&nbsp;&nbsp;&nbsp;' +
		'<input id="remote_1" type="radio" name="isremote" value="1" onclick="ReLo(1, \'upgallery\');"/><label for="remote_1"> '+L['up_remote']+
		'</label></td></tr><tr id="remote_url" style="display:none;"><td><input id="remote" type="text" size="30" name="remote" value="http://" onclick="if(this.value==\'http://\')this.value=\'\';"/>' +
		'</td></tr><tr id="local_url"><td><input id="upgallery" type="file" size="20" name="upgallery" ' +
		'onchange="if(isImg(\'upgallery\')){this.form.submit();Dd(\'Dsubmit\').disabled=true;Dd(\'Dsubmit\').value=\''+L['uploading']+'\';}"/>' +
		'</td></tr><tr style="display:'+s+'"><td>'+L['width']+' <input type="text" size="3" name="width" value="'+w+'"/> px &nbsp;&nbsp;&nbsp;'+L['height']+
		' <input type="text" size="3" name="height" value="'+h+'"/> px </td></tr><tr><td><input type="submit" class="btn" value="'+L['upload']+'" id="Dsubmit"/>' +
...
var _global_i = 1;
function getAlbum(v, i) {i = _global_i; Dd('thumb'+i).value = v; Dd('showthumb'+i).src = DTPath + v;}
...
function selAlbum(i) {_global_i = i;Dwidget(DTPath+'member/image.php?editor=1', L['choose_img'], 950, 500, 'auto')}
function ReLo(r, i) {
	if(r) {
...
function tpl_edit(f, d, i) {var v = Dd('gaosou_template_'+i).firstChild.value; var n = v ? v : f; Dwidget('?file=template&action=edit&fileid='+n+'&dir='+d, L['tpl_edit']);}
