...
abstract class time_base_class {
	...
	protected function excutecommon(){
		if ($this->_a_uedit_arr){
				$user_sql = "update yw_company set balance = balance+case userid";
				$t_temp_sql = "";
				$t_task_tids = array();
				...
				$user_sql .= " where userid in (".implode(',',$t_user_uids).")";
				db_factory::execute($user_sql);
		}
		if ($this->_a_notify_arr){
			$noti_sql = "insert into ".TB_PRE."witkey_message (title,content,recive_uid,recive_username,on_time) values";
			$i =0;
		...
		if ($this->_a_feed_arr){
			$feed_sql = "insert into ".TB_PRE."witkey_feed (title,uid,username,feedtype,obj_id,feed_time) values";
			$i =0;
		...
		if ($this->_a_fina_arr){
			$fina_sql = "insert into ".TB_PRE."finance_record (reason,uid,username,obj_id,amount,balance,addtime,site_profit)  values ";
			$i =0;
			foreach ($this->_a_fina_arr as $fina){
				if ($i++){
					$fina_sql.=",";
				}
				$fina['fina_type'] == 'in' ? $fina_type = 1 : $fina_type = -1;
				$fina_sql .="('".$fina['reason']."','".($fina['uid']+0)."','{$fina['username']}','".($fina['task_id']+0)."','".($fina['amount']*$fina_type)."','".($fina['balance']+0)."','".($fina['addtime']+0)."','".($fina['site_profit']+0)."')";
			}
			db_factory::execute($fina_sql);
			db_factory::execute("update ".TB_PRE."finance_record set unique_num = CONCAT('88',LPAD(itemid,8,'0')) where !ifnull(unique_num,0) ");
		}
	}
}
?>