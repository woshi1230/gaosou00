...
abstract class keke_auth_base_class {
	...
	public static  function get_auth_item($auth_code=null,$find_str=null,$is_open=false,$w=null,$cache=true) {
		global $_cache_obj;
			...
			$auth_item=kekezu::get_table_data($fds,TB_PRE . "witkey_auth_item", $where,'listorder asc','','','auth_code',$c);
	...
	public function set_auth_record_status($uid,$status) {
		return db_factory::execute(sprintf(" update %switkey_auth_record set auth_status = '%d' where auth_code= '%s' and uid = '%d'",TB_PRE,$status,$this->_auth_code,$uid));
	}
	public function set_auth_status($auth_id,$status){
		return db_factory::execute(sprintf(" update %s set auth_status = '$status' where %s = '%d'",TB_PRE.$this->_auth_table_name,$this->_primary_key,$auth_id));
	}
	public function add_auth_record($uid,$username, $auth_code,$end_time, $data = array(),$auth_status='0') {
		...
		$record_info = db_factory::get_one(sprintf(" select * from %switkey_auth_record where uid = '%d' and auth_code = '%s'",TB_PRE,$uid,$auth_code));
	...
	public function del_auth_record($uid) {
		return db_factory::execute(sprintf(" delete from %switkey_auth_record where uid= '%d' and auth_code = '%s'",TB_PRE,$uid,$this->_auth_code));
	}
	...
	public function get_auth_info($auth_ids){
		if(isset($auth_ids)){
			if(!stristr($auth_ids,',')) {
			 	return  db_factory::query(sprintf(" select * from %s where %s = '%s'",TB_PRE.$this->_auth_table_name,$this->_primary_key,$auth_ids));
			}else{
				return db_factory::query(sprintf(" select * from %s where %s in (%s) ",TB_PRE.$this->_auth_table_name,$this->_primary_key,$auth_ids));
			}
		}else{
			return array();
		}
	...
	public function get_user_auth_info($uid,$is_username=0,$show_id=''){
		$sql="select * from ".TB_PRE.$this->_auth_table_name;
		if($uid){
	...
	public function set_user_role($uid,$action='pass'){
		...
		db_factory::execute(sprintf(" update yw_company set user_type='%d' where userid='%d'",$user_role,$uid));
		db_factory::execute(sprintf(" update %switkey_shop set shop_type='%d' where uid='%d'",TB_PRE,$user_role,$uid));
	}
	private function extract_birth($uid,$idcard){
		...
		$f = $y.'-'.$m.'-'.$d;
		db_factory::execute(sprintf("update yw_company set birthday='%s' where userid='%d'",$f,$uid));
	}
}