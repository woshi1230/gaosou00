...
class keke_curren_class {
	...
	public static function get_curr_list($code='*'){
		return self::$_currencies = kekezu::get_table_data ($code, TB_PRE . 'witkey_currencies', '', '', '', '', 'code', 3600 );
	}
		...
		if ($mulit == true) {
			...
			if ($ex) {
				if ($ex == self::$_curr) {
					...
					$rate and $res = db_factory::execute ( sprintf ( " update %switkey_currencies set value='%.8f',last_updated='%s' where code='%s'", TB_PRE, $rate, time (), $ex ) );
				}
			}
		}
		return $res;
