<?php
keke_lang_class::load_lang_class('yw_task_agreement');
abstract class yw_task_agreement {
	public $_agree_id;
	...
	public static function create_agreement($agree_title, $mode_id, $task_id, $work_id, $buyer_uid, $seller_uid) {
		$agree_obj = new Yw_witkey_agreement_class ();
		...
		return $agree_obj->create_yw_witkey_agreement ();
	}
	public function get_agreement_info() {
		global $_K, $uid, $kekezu;
		$agree_info = db_factory::get_one ( sprintf ( " select * from %switkey_agreement where agree_id = '%d'", TB_PRE, $this->_agree_id ) );
	...
	public function buyer_contact_init() {
		$info = db_factory::get_one ( sprintf ( " select a.contact,a.username,b.truename,b.mobile,b.email,b.qq,b.msn from %switkey_task a left join yw_member b on a.uid=b.userid where a.task_id='%d'", TB_PRE, $this->_task_id ) );
	...
	public function seller_contact_init() {
		$info = db_factory::get_one ( sprintf ( " select truename,username,qq,mobile,email,msn from yw_member where userid='%d'", $this->_seller_uid ) );
			...
			switch ($user_type) {
				case "1" :
					if ($seller_sattus == '2') {
						...
						if ($res) {
							db_factory::execute ( sprintf ( " update %switkey_agreement set seller_accepttime='%s' where seller_uid='%d' and agree_id ='%d'", TB_PRE, time (), $this->_seller_uid, $this->_agree_id ) );
							switch ($buyer_status) {
								case "1" :
				...
				case "2" :
					if ($buyer_status == '2') {
						...
						if ($res) {
							db_factory::execute ( sprintf ( " update %switkey_agreement set buyer_accepttime='%s' where buyer_uid ='%d' and agree_id='%d'", TB_PRE, time (), $this->_buyer_uid, $this->_agree_id) );
							switch ($seller_sattus) {
								case "1" :
									$notice="买家协议签署完成，等待高手签署协议";
									break;
								case "2" :
									$notice = $_lang['agreement_signed_complete_to_deliver'];
									$this->set_agreement_status ( 'agree_status', "2" );
	...
	public function upfile_confirm($file_ids) {
		...
		$res = db_factory::execute ( sprintf ( " update %switkey_agreement set seller_confirmtime = UNIX_TIMESTAMP(),file_ids = '%s' where agree_id='%d'", TB_PRE, $file_ids, $this->_agree_id ) );
	...
	public function accept_confirm() {
		...
		if ($this->_agree_status == 2 && $this->_seller_status == 3 && $this->_buyer_status == 3) {
				$res = db_factory::execute ( sprintf ( " update %switkey_agreement set buyer_confirmtime = UNIX_TIMESTAMP() where agree_id ='%d'", TB_PRE, $this->_agree_id ) );
				db_factory::execute ( sprintf ( " update %switkey_task set task_status = '8' where task_id ='%d'", TB_PRE, $this->_task_id ) );
	...
	public function get_file_list() {
		if ($this->_agree_info ['file_ids']) {
			...
			return db_factory::query ( sprintf ( " select file_id,file_name,save_name from %switkey_file where file_id in(%s)", TB_PRE, $fileIds ) );
		}
	}
	...
	public function set_agreement_status($type = 'agree_status', $to_status) {
		return db_factory::execute ( sprintf ( " update %switkey_agreement set %s = '%d' where agree_id = '%d'", TB_PRE, $type, $to_status, $this->_agree_id ) );
	}
