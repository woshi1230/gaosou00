...
class keke_order_class {
	public static function get_order_info($order_id) {
		...
		return db_factory::get_one ( sprintf ( $sql, TB_PRE, TB_PRE, $order_id ) );
	}
	public static function get_order_id($obj_type, $obj_id) {
		...
		return db_factory::get_count ( sprintf ( $sql, TB_PRE, $obj_type, $obj_id ) );
	}
	public static function get_order_detail($order_id) {
		$sql = "select * from %switkey_order_detail where order_id = '%d'";
		return db_factory::query ( sprintf ( $sql, TB_PRE, $order_id ) );
	}
	public static function create_order($model_id, $seller_uid, $seller_username, $order_name, $order_amount, $order_body, $order_status = 'ok',$to_seller_message = null) {
		global $uid, $username;
		$order_obj = new Yw_witkey_order_class ();
		...
		return $order_obj->create_yw_witkey_order ();
	}
	public static function create_user_charge_order($order_type, $pay_type, $money, $obj_id = null, $pay_info = '', $order_status = 'wait', $uid = '', $username = '') {
		...
		$order_info = db_factory::get_one ( sprintf ( $sql, TB_PRE, $uid, $pay_type ) );
		...
		$update and db_factory::execute ( sprintf ( " update %switkey_order_charge set pay_money='%.2f',pay_time='%s' where order_id='%d'", TB_PRE, $money, time (), $order_id ) );
		if ($create) {
	...
	public static function update_order_charge($out_trade_no,$pay_money){
		$order_info = db_factory::get_one ( sprintf ( "select * from %switkey_order_charge where out_trade_no ='%s'", TB_PRE, $out_trade_no ) );
		if($order_info ['order_status'] == 'wait'){
			db_factory::execute ( sprintf ( " update %switkey_order_charge set pay_money='%.2f',pay_time='%s' where order_id='%d'", TB_PRE, $pay_money, time (), $order_info ['order_id'] ) );
		}
	...
	public static function create_order_charge($order_type,$pay_type,$return_order_id,$obj_id,$uid,$username,$pay_money,$order_status,$pay_info,$out_trade_no,$pay_time =NULL,$attach = NULL){
		$order_obj = new Yw_witkey_order_charge_class ();
		$order_obj->_order_id = null;
	...
	public static function create_order_detail($order_id, $detail_name, $obj_type, $obj_id, $price, $num = '1',$detail_type=null) {
		...
		return $detail_obj->create_yw_witkey_order_detail ();
	}
	...
	public static function del_order($order_id, $url = '', $output = 'normal') {
		global $_lang;
		$res = db_factory::execute ( sprintf ( " delete from %switkey_order where order_id='%d'", TB_PRE, $order_id ) );
		$res *= db_factory::execute ( sprintf ( " delete from %switkey_order_detail where order_id = '%d'", TB_PRE, $order_id ) );
		if($res){
			return true;
		}else{
			return false;
	...
	public static function set_order_status($order_id, $to_status) {
		return db_factory::execute ( sprintf ( " update %switkey_order set order_status='%s' where order_id='%d'", TB_PRE, $to_status, $order_id ) );
	}
	public static function order_cancel_return($order_id) {
		$fina_info = db_factory::get_one ( sprintf ( " select uid,amount,0 fina_credit from %sfinance_record where order_id ='%d'", TB_PRE, $order_id ) );
		if ($fina_info) {
			keke_finance_class::init_mem('order_cancel', array(':order_id'=>$order_id));
			return keke_finance_class::cash_in ( $fina_info ['uid'], $fina_info ['amount'], "order_cancel", '', 'order', $order_id );
		} else {
			return true;
		}
	}
	...
	public static function update_fina_order($fina_id, $order_id) {
		return db_factory::execute ( sprintf ( " update %sfinance_record set order_id = '%d' where itemid = '%d'", TB_PRE, $order_id, $fina_id ) );
	}
