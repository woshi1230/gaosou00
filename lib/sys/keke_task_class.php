...
abstract class keke_task_class {
	...
	public function get_search_condit() {
		...
		$search_condit = kekezu::get_table_data ( " count($fields_arr[pk]) count,$fields_arr[st]", TB_PRE . $fields_arr ['tab'], " task_id='" . $this->_task_id . "' ", "", $fields_arr ['st'], "", $fields_arr ['st'] );
		return $search_condit;
	}
	public function get_task_file() {
		return kekezu::get_table_data ( "*", TB_PRE . "witkey_file", " obj_type = 'task' and task_id = '" . $this->_task_id . "'", "", "", "", "file_id", 3600 );
	}
	public function get_task_related($same = true, $lasted = true) {
		$related = array ();
		if ($same) {
			$same_task = db_factory::query ( sprintf ( " select task_title,task_id,task_cash,task_cash_coverage,model_id from %switkey_task where indus_id = '%d' and model_id='%d' and task_id!='%d' and task_status in(2,3) and focus_num>0 order by task_cash desc limit 0,5", TB_PRE, $this->_task_info ['indus_id'], $this->_model_id, $this->_task_id ) );
			$same_task and $related ['same'] = $same_task;
		}
		if ($lasted) {
			$favor_task = db_factory::query ( sprintf ( " select task_title,task_id,task_cash,task_cash_coverage,model_id from %switkey_task where indus_id = '%d' and model_id='%d' and task_status in(2,3) and task_id!='%d' order by view_num desc limit 0,5", TB_PRE, $this->_task_info ['indus_id'], $this->_model_id, $this->_task_id ) );
	...
	public static function get_comment($obj_type, $task_id, $obj_id, $work_uid) {
		global $uid;
//		$condit = "  from " . TB_PRE . "comment where obj_type='$obj_type' and obj_id='$obj_id' and origin_id='$task_id'";
		$condit = "  from " . TB_PRE . "comment where obj_type='$obj_type' and item_id='$obj_id'";
		$comment_arr = db_factory::query ( " select * " . $condit );
		if ($comment_arr && $work_uid == $uid) {
			db_factory::execute ( sprintf ( " update %scomment set status = '1' where item_id= '%d'", TB_PRE, $obj_id ) );
		}
		return $comment_arr;
	}
	public static function get_task_comment($task_id, $url, $page) {
		$comment_obj = keke_table_class::get_instance ( "comment" );
		return $comment_arr = $comment_obj->get_grid ( "item_id='$task_id' and obj_type='task' and qid=0 order by itemid desc", $url, $page, 10, null, 1, 'task_leave' );
	}
		...
		if ($delay_cash > $mycredit + $mycash) {
			...
			if ($delay_id) {
				...
				db_factory::execute ( sprintf ( " update %switkey_task set end_time=end_time+'%s',sub_time=sub_time+'%s',is_delay =ifnull(is_delay,0)+1
							,credit_cost=credit_cost+'%s',cash_cost=cash_cost+'%s',real_cash=real_cash+'%s',task_cash=task_cash+'%s' where task_id='%d'", TB_PRE, $add_time, $add_time, $credit_cost, $cash_cost, $real_cash_add, $delay_cash, $this->_task_id ) );
				db_factory::updatetable ( TB_PRE . "witkey_order", array (
				...
				if ($this->_model_id == 3) {
					...
					db_factory::execute ( sprintf ( "update %switkey_task set single_cash = single_cash+'%s' where task_id='%d'", TB_PRE, $add_single, $this->_task_id ) );
				}
	...
	public static function set_task_comment($comment_arr, $is_reply = false) {
		...
		$lid = db_factory::inserttable ( TB_PRE . "comment", $comment_arr, 1 );
		$is_reply or db_factory::execute ( sprintf ( "update %switkey_task set leave_num = leave_num+1 where task_id = '%d'", TB_PRE, $comment_arr ['obj_id'] ) );
		return $lid;
	}
	public static function get_comment_info($comment_id) {
		$commnet_obj = new Yw_comment_class ();
		$commnet_obj->setWhere ( "itemid = $comment_id" );
		$comment_info = $commnet_obj->query_yw_comment ();
		$comment_info = $comment_info [0];
		return $comment_info;
	}
	public static function get_taskstatus($task_id) {
		$staus_arr = db_factory::get_one ( sprintf ( "select task_status from %switkey_task where task_id = '%d'", TB_PRE, $task_id ) );
		return $staus_arr ['task_status'];
	}
	public function get_user_special($uid, $special_fields = 'mobile,email') {
		return db_factory::get_one ( sprintf ( " select %s from yw_company where userid = '%d' ", $special_fields, $uid ) );
	}
	public function get_task_work($work_id = '', $work_status = '') {
		...
		$st = $fields_arr ['st'];
		if ($work_id) {
			return db_factory::get_one ( sprintf ( " select * from %s%s where %s='%d'", TB_PRE, $tab, $pk, $work_id ) );
		} else {
			$where = " 1 = 1 and task_id = '" . $this->_task_id . "' ";
			strpos ( $work_status, "," ) == FALSE and $where .= " and $st = '$work_status' " or $where .= " and $st in ($work_status) ";
			return kekezu::get_table_data ( "*", TB_PRE . $tab, $where );
		}
	}
	public static function get_work_file($file_ids) {
		$sql = " select * from " . TB_PRE . "witkey_file where obj_type='work' and  file_id ";
		strpos ( $file_ids, "," ) !== FALSE and $sql .= " in (" . $file_ids . ")" or $sql .= " = '$file_ids' ";
		return db_factory::query ( $sql );
	}
	public function get_mark_count() {
		return kekezu::get_table_data ( " count(mark_id) count,mark_status", TB_PRE . "witkey_mark", "model_code='" . $this->_model_code . "' and origin_id='$this->_task_id'", "", "mark_status", "", "mark_status", 3600 );
	}
	public function get_mark_count_ext() {
		return kekezu::get_table_data ( " count(mark_id) c,mark_type", TB_PRE . "witkey_mark", "model_code='" . $this->_model_code . "' and origin_id='$this->_task_id' and mark_status>0", "", "mark_type", "", "mark_type" );
	}
	public function set_task_status($to_status) {
		return db_factory::execute ( sprintf ( " update %switkey_task set task_status='%d' where task_id='%d'", TB_PRE, $to_status, $this->_task_id ) );
	}
	...
	public function set_work_comment($obj, $work_id, $content) {
		global $_lang, $kekezu;
		$comment_obj = new Keke_comment_class ();
		...
		$comment_id = $comment_obj->create_keke_comment ();
		if ($comment_id) {
	...
	public function del_work($work_id, $url = '', $output = 'normal') {
		global $user_info;
		global $_lang;
		$user_info ['groupid'] != '7' and kekezu::keke_show_msg ( $url, $_lang ['you_no_rights_delete_manuscript'], "error", $output );
		$fields_arr = $this->get_work_fields ();
		$tab = $fields_arr ['tab'];
		$pk = $fields_arr ['pk'];
		$st = $fields_arr ['st'];
		$res = db_factory::execute ( sprintf ( " delete from %s%s where %s = '%d'", TB_PRE, $tab, $pk, $work_id ) );
		if ($res) {
			db_factory::execute ( sprintf ( "update %switkey_task set work_num = work_num-1 where task_id = '%d'", TB_PRE, $this->_task_id ) );
	...
	public function has_new_comment($page, $limit) {
		...
		$pk = $fields_arr ['pk'];
		$pre = ($page - 1) * $limit;
		$work_ids = kekezu::get_table_data ( $pk, TB_PRE . $tab, " task_id ='$this->_task_id'", "", "", " $pre,$limit", $pk );
		if ($work_ids) {
//			$new_arr = kekezu::get_table_data ( "count(itemid) count,obj_id", TB_PRE . "comment", " item_id in (" . implode ( ",", array_keys ( $work_ids ) ) . ") and status=0 and origin_id='$this->_task_id'", "", " obj_id ", "", "obj_id" );
			$new_arr = kekezu::get_table_data ( "count(itemid) count,obj_id", TB_PRE . "comment", " item_id in (" . implode ( ",", array_keys ( $work_ids ) ) . ") and status=0", "", " item_id ", "", "item_id" );
		}
		return $new_arr;
	}
	public function notify_user($action, $title, $v_arr = array(), $notify_type = 2, $uid = null,$msg_type=1) {
		...
		$this->_msg_obj->send_message ( $user_info ['userid'], $user_info ['username'], $action, $title, $v_arr, $user_info ['mail'], $user_info ['telephone'],$msg_type );
	}
	...
	public function check_if_good_mark($mark_id, $to_status, $url = '', $output = 'normal') {
		global $_lang;
		$mark_status = db_factory::get_count ( sprintf ( " select mark_status from %switkey_mark where mark_id='%d'", TB_PRE, $mark_id ) );
	...
	public function plus_view_num() {
		if (! $_SESSION ['task_views_' . $this->_task_id . '_' . $this->_uid] && $this->_uid != $this->_guid) {
			...
			return db_factory::execute ( sprintf ( 'update %switkey_task set view_num=view_num+1 where task_id=%d', TB_PRE, $this->_task_id ) );
		}
	}
	public function plus_work_num() {
		return db_factory::execute ( sprintf ( "update %switkey_task set work_num=work_num+1 where task_id ='%d'", TB_PRE, $this->_task_id ) );
	}
	public function plus_accepted_num($uid) {
		return db_factory::execute ( sprintf ( "update yw_company set accepted_num = accepted_num+1 where userid='%d'", intval ( $uid ) ) );
	}
	public function plus_take_num() {
		return db_factory::execute ( sprintf ( "update yw_company set take_num = take_num+1 where userid ='%d'", $this->_uid ) );
	}
	public function plus_leave_num() {
		return db_factory::execute ( sprintf ( "update %switkey_task set leave_num=leave_num+1 where task_id ='%d'", TB_PRE, $this->_task_id ) );
	}
	public function plus_focus_num() {
		return db_factory::execute ( sprintf ( "update %switkey_task set focus_num=focus_num+1 where task_id ='%d'", TB_PRE, $this->_task_id ) );
	}
	public function plus_work_comment_num($work_id) {
		$fields_arr = $this->get_work_fields ();
		$tab = $fields_arr ['tab'];
		$pk = $fields_arr ['pk'];
		return db_factory::execute ( sprintf ( "update %s%s set comment_num=comment_num+1 where %s ='%d'", TB_PRE, $tab, $pk, $work_id ) );
	}
	public function plus_mark_num() {
		return db_factory::execute ( sprintf ( "update %switkey_task set mark_num=ifnull(mark_num,0)+2 where task_id ='%d'", TB_PRE, $this->_task_id ) );
	}
	...
	public static function hp_timeout($days = 7) {
		$mark_list = db_factory::query ( sprintf ( "select * from %switkey_mark where mark_status=0 and mark_time<'%d'-'%d'", TB_PRE, time (), 7 * 24 * 3600 ) );
	...
	function has_mark($obj_ids) {
		...
		$data = array ();
		if ($uid && $obj_ids) {
			$data = kekezu::get_table_data ( "mark_id,obj_id,mark_status,mark_count", TB_PRE . "witkey_mark", "mark_status>0 and by_uid='$uid' and origin_id='$this->_task_id' and obj_id in ($obj_ids)", "", "", "", "obj_id" );
		}
		return $data;
	}
	...
	public function work_pic($file_ids) {
		...
		$info = db_factory::query ( ' select file_name,save_name from ' . TB_PRE . 'witkey_file where file_id in (' . $file_ids . ')' );
		$pic = array ();
		foreach ( $info as $v ) {
	...
	function check_work_hide() {
		global $uid;
		$work_info = db_factory::get_one ( sprintf ( "select * from %switkey_task_work where hide_work=1 and uid=%d and task_id=%d", TB_PRE, $uid, $this->_task_id ) );
		return $work_info;
	}
	function getRecentWorkInfo() {
		global $uid;
		return db_factory::get_one ( sprintf ( "select * from %switkey_task_work where uid=%d and task_id=%d order by work_time desc limit 0,1", TB_PRE, $uid, $this->_task_id ) );
	}
	...
	public function task_top_end(){
		...
		if (time () > $arrRecordInfo ['end_time']) {
			db_factory::execute("update ".TB_PRE."witkey_task set tasktop=0 where task_id=".$this->_task_id);
		}
	}
}