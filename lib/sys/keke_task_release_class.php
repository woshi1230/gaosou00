...
//			}
	...
	private function submit_check() {
		...
		if ($r_info) {
			if (! $r_info [txt_title] || ! $r_info [fid]) {
	...
	public function public_pubtask() {
		...
		$ipinfo = db_factory::get_one ( "select * from " . TB_PRE . "area where areaname='" . $ipinfo ['data'] ['region'] . "'" );
...
//		$task_obj->setTask_status ( 0 );
	...
	public function update_task_info($task_id, $obj_name) {
		...
		$user_info = $this->_user_info; 
		$task_obj = $this->_task_obj; 
		db_factory::execute ( "update yw_company set pub_num = pub_num+1 where userid=$this->_uid " );
...
//		CustomClass::createExtData ( $task_id, $this->_model_id, $release_info );
	...
	public function create_task_order($task_id, $model_id, $release_info, $order_status = 'ok') {
		...
		if ($model_id == 16) {
			...
			db_factory::updatetable ( TB_PRE . 'witkey_task', array (
					'task_status' => $status 
			), array (
					'task_id' => $task_id 
			) );
	...
	public function check_access($r_step, $model_id, $release_info, $task_id = null, $output = 'normal') {
		...
		switch ($r_step) {
			...
			case "step3" : 
				...
				break;
			case "step4" : 
				$sql = sprintf ( " select * from %switkey_task where task_id = '%d' and model_id = '%d'", TB_PRE, $task_id, $model_id );
	...
	public function checkWhetherToPay($intTaskId) {
		$floatBalance = floatval ( $this->_user_info ['money'] );
	...
	public function getTaskInfoByTaskId($intTaskId, $strFields = NULL) {
		...
		return db_factory::get_one ( ' select ' . $strFields . ' from ' . TB_PRE . 'witkey_task where task_id = ' . intval ( $intTaskId ) );
	}
}