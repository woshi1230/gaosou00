...
class pay_batch_fac_class {
	...
	public function alipayjs_success_notify($detail_arr, $status = true) {
		...
		$info = kekezu::get_table_data ( "withdraw_id,uid,username,withdraw_status,withdraw_cash,pay_username,pay_account", TB_PRE . "witkey_withdraw", " withdraw_id in ($ids)", "", "", "", "withdraw_id" );
			...
			if ($info [$k] ['withdraw_status'] == 1) {
				switch ($v ['status']) {
					case "S" :
						...
						$res = db_factory::execute ( sprintf ( " update %switkey_withdraw set withdraw_status='2',fee=%.2f where withdraw_id ='%d'", TB_PRE,$fee, $k ) );
						...
						break;
					case "F" :
						$res = db_factory::execute ( sprintf ( " update %switkey_withdraw set withdraw_status='3' where withdraw_id ='%d'", TB_PRE, $k ) );
