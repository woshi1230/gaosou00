...
class keke_trust_fac_class {
			...
			case "sns_bind":
				switch ($this->_data ['is_success']) {
					case "T" :
						...
						$username = db_factory::get_count(sprintf("select username from yw_member where userid='%d'", $uid));
						$key = $this->_data['key'];
						$account = $this->_data['alipay_login_id'];
						$oauth_id = $this->_data['alipay_card_no'];
						$id = db_factory::get_count ( sprintf ( " select id from %switkey_member_oauth where uid='%d' and username='%d' and source='alipay_trust'", TB_PRE, $uid, $username ) );
						if ($id) {
							$sql = " update %switkey_member_oauth set account='%s',oauth_id='%s',bind_key='%s' where id='%d'";
							$res = db_factory::execute ( sprintf ( $sql, TB_PRE, $account, $oauth_id, $key, $id ) );
						} else {
							$sql = " insert into %switkey_member_oauth values('','alipay_trust','%s','%d','%s','%d','%s','%s')";
							$res = db_factory::execute ( sprintf ( $sql, TB_PRE, $account, $uid, $username, time (), $oauth_id, $key ) );
						}
							break;
						case "F" :
							$res = false;
			...
			case "cancel_bind":
				switch ($this->_data ['is_success']) {
						case "T" :
							...
							$res = db_factory::execute ( sprintf ( $sql, TB_PRE, $uid ) );
							break;
						case "F" :
							$res = false;
							break;
	...
	public static function set_task_status($task_id,$to_status) {
		return db_factory::execute ( sprintf ( " update %switkey_task set task_status='%d' where task_id='%d'", TB_PRE, intval ( $to_status ),$task_id) );
	}
	...
	public static function verify_bind($uid, $bind_type) {
		return db_factory::get_one( sprintf ( " select uid,oauth_id from %switkey_member_oauth where uid = '%d' and source='%s'", TB_PRE, $uid, $bind_type ) );
	}
