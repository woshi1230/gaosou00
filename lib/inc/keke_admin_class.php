...
class keke_admin_class {
	...
	static function get_admin_menu() {
		...
		if (! $menuset_arr) {
			...
			foreach ( $model_list as $model ) {
				$init_menu = array ();
				if (! file_exists ( S_ROOT . $model ['model_type'] . '/' . $model ['model_code'] . '/admin/init_config.php' )) {
					continue;
				}
				require S_ROOT . $model ['model_type'] . '/' . $model ['model_code'] . '/admin/init_config.php';
				$mulist_arr = array ();
	...
	function screen_unlock($unlock_num, $unlock_pwd) {
		...
		if ($unlock_num > 0) { 
			$admin_pwd = db_factory::get_count ( " select password from yw_member where userid = '" . $_SESSION ['uid'] . "'" );
	...
	function search_nav($ser_resource) {
		$resource_info = db_factory::query ( " select resource_name,resource_url from " . TB_PRE . "witkey_resource where INSTR(resource_name,'$ser_resource') > 0 " );
		if ($resource_info)
			return $resource_info;
		else {
			return db_factory::query ( "select resource_name,resource_url from " . TB_PRE . "witkey_resource a left join " . TB_PRE . "witkey_resource_submenu b
			 on a.submenu_id = b.submenu_id where INSTR(b.submenu_name,'$ser_resource')>0" );
		}
	}
	public function get_shortcuts_list() {
		return db_factory::query ( " select b.resource_id,b.resource_name,b.resource_url from " . TB_PRE . "witkey_shortcuts a left join " . TB_PRE . "witkey_resource b on a.resource_id = b.resource_id where a.uid = '$this->_uid' order by a.s_id desc " );
	}
	function add_fast_menu($r_id) {
		global $_lang;
		$shortcuts_obj = new Keke_witkey_shortcuts_class ();
		$in_shortcuts_list = db_factory::execute ( " select resource_id from " . TB_PRE . "witkey_shortcuts where resource_id = '$r_id'" );
	...
	function rm_fast_menu($r_id) {
		global $_lang;
		$shortcuts_obj = new Keke_witkey_shortcuts_class ();
		$shortcuts_list = db_factory::get_one ( " select uid,resource_id from " . TB_PRE . "witkey_shortcuts where resource_id = '$r_id' and uid = '$this->_uid'" );
		if ($shortcuts_list) {
			if ($shortcuts_list ['uid'] != $this->_uid) {
				kekezu::echojson ( $_lang['not_delete_others_shortcuts'], '2' );
			} else {
				$success = db_factory::execute ( " delete from " . TB_PRE . "witkey_shortcuts where resource_id = '$r_id' and uid = '$this->_uid'" );
				if ($success) {
	...
	static function get_article_cate() {
		return kekezu::get_table_data ( "*", TB_PRE . "witkey_article_category", "", "listorder asc ", "", "", "art_cat_id", null );
	}
	public function admin_login($username, $password, $allow_times, $formhash='') {
		...
		global $kekezu;
			...
			if (! $user_info) {
				...
				die ();
			} else if (! $user_info ['groupid'] && $user_info ['userid'] != ADMIN_UID) {
					$kekezu->echojson ( $_lang['no_rights_login_backstage'], "2", array('formhash'=>$hash) );
				die ();
			} else {
				$_SESSION ['auid'] = $_SESSION ['uid'] = $user_info ['userid'];
	...
	public static function get_withdraw_cash($cash){
		$fee = floatval ( $cash );
		$pay_config = kekezu::get_table_data ( "*", TB_PRE . "witkey_pay_config", '', '', "", '', 'k' );
