...
class UserCenter {
	public static function getIndusById($intIndusId){
		return db_factory::get_table_data('*',TB_PRE . 'witkey_industry',' indus_pid = '.intval($intIndusId),'CASE WHEN listorder = 0 THEN 9999999 WHEN listorder > 0 THEN listorder END','','','indus_id',3600);
	}
	public static function bindingAccount($uid,$username,$arrSaveinfo){
		$returnValue = FALSE;
		$strSql = "select count(id) from ".TB_PRE."witkey_member_oauth
...
public static function deleteAccount($uid,$arrSaveinfo){
	db_factory::execute("delete  from ".TB_PRE."witkey_member_oauth where uid='".intval($uid)."' and source ='".$arrSaveinfo['type']."'");
}
