...
class keke_prom_class {
	...
	public static function get_prom_rule($prom_code) {
		$p_config = db_factory::get_one ( sprintf ( " select * from %switkey_prom_rule where prom_code='%s'", TB_PRE, $prom_code ) );
	...
	public function get_prom_relation($uid, $prom_type) {
		...
		$p_relation = db_factory::get_one ( sprintf ( $sql, TB_PRE, $uid, $prom_type ) );
		if (! $p_relation) { 
			$p_relation or $p_relation = db_factory::get_one ( sprintf ( $sql, TB_PRE, $uid, 'reg' ) );
	...
	function get_prom_event($obj_id, $uid, $action, $event_status = '1') {
		...
		return db_factory::get_one ( sprintf ( $sql, TB_PRE, TB_PRE, $obj_id, $action, $uid, $event_status ) );
	}
	...
	function create_prom_event($action, $uid, $obj_id, $cash = 0, $credit = 0, $event_status = '1') {
		$result = FALSE;
		$arrTaskInfo = db_factory::get_one(sprintf("select * from %switkey_task where task_id = %d",TB_PRE,$obj_id));
		$arrBidInfo = db_factory::get_one(sprintf("select * from %switkey_task_bid where task_id = %d and bid_status = 4",TB_PRE,$obj_id));
		if ($this->_prom_open) {
		...
		if ($prom_event) {
			if($prom_event['action']=='reg'){
				if(!$this->_auth){
					...
					$count = db_factory::get_count(sprintf('select count(*) from %s where uid = %d and auth_code in ("%s") and auth_status = 1',TB_PRE.'witkey_auth_record' ,$prom_event['uid'],$auth));
	...
	function set_relation_status($relation_id, $status) {
		return db_factory::execute ( " update " . TB_PRE . "witkey_prom_relation set relation_status ='$status' where relation_id ='$relation_id'" );
	}
	function set_prom_event_status($p_uid, $username, $event_id, $status) {
		global $_lang;
		$prom_info = db_factory::get_one ( "select * from " . TB_PRE . "witkey_prom_event where event_id = " . intval ( $event_id ) );
		if ($prom_info) {
			...
			$res = db_factory::updatetable(TB_PRE . 'witkey_prom_event', array('event_status'=>intval($status)),array('event_id'=>intval($event_id)));
			if ($res) {
				if ($status == 2) {
	...
	function prom_income_rank() {
		return kekezu::get_table_data ( " uid,username,sum(abs(amount)) cash,0 credit", TB_PRE . "finance_record", " INSTR(reason,'prom_')", "", "uid", "", "uid", 3600 );
	}
	...
	public function is_meet_requirement($prom_code, $obj_id) {
		...
		if ($obj_info) {
			$prom_config = db_factory::get_one ( sprintf ( " select * from %switkey_prom_rule where prom_code='%s'", TB_PRE, $prom_code ) );
	...
	public static function get_prom_obj_info($prom_type, $obj_id) {
		if ($prom_type == 'pub_task' || $prom_type == 'bid_task') {
			$obj_info = db_factory::get_one ( sprintf ( " select model_id,indus_id,profit_rate,task_cash cash from %switkey_task where task_id='%d'", TB_PRE, $obj_id ) );
		} elseif ($prom_type == 'service') {
			$obj_info = db_factory::get_one ( sprintf ( " select model_id,indus_id,profit_rate,price cash from %switkey_service where service_id='%d'", TB_PRE, $obj_id ) );
		}
		return $obj_info;
	}
	...
	public static function get_prom_type() {
		return kekezu::get_table_data ( "prom_code,prom_item,type", TB_PRE . "witkey_prom_rule", "", "", "", "", "prom_code", 3600 );
	}
}