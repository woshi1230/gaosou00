...
class keke_base_class {
	...
	static function redirect_second_domain() {
		...
		if ($kekezu->_sys_config ['second_domain']) { 
			...
			if ($m [1]) { 
				$uid = intval ( $m [1] );
				$e = db_factory::query ( sprintf ( ' select uid from yw_member where userid=%d', $uid ) );
				if ($e) {
	...
	static function time_js($mode = 'set', $timer = 300) {
	...
	static function get_rand_kf() {
		$kf_arr = kekezu::get_table_data ( 'userid', 'yw_company', ' group_id = 7' );
