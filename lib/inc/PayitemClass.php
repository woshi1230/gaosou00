...
class PayitemClass {
	...
	public static function getPayitemList ($strType='task',$pk=null,$intIsOpen=1,$strUncode=null){
		if(TOOL === TRUE){
			...
			$pk         or $pk = "item_code";
			$arrPayitemList = kekezu::get_table_data ( "*", TB_PRE . "witkey_payitem", "1=1 $strWhere ", "", "", "", $pk,3600 );
  ...
  public static function getPayitemRecord($strType='task',$objId,$code){
  	if( TOOL === TRUE){
      return db_factory::get_one(sprintf("select * from ".TB_PRE."witkey_payitem_record where obj_type='%s' and obj_id=%d and item_code='%s'",$strType,$objId,$code));
  	}
  }
	...
	public static function getPayitemOrderAmountByObjId($objId,$objType = 'task'){
		...
		$arrOrderDetailInfo = db_factory::get_one(sprintf("select order_id from ".TB_PRE."witkey_order_detail where obj_id =%d and obj_type = '%s' and detail_type in(%s) order by detail_id desc limit 0,1",$objId,$objType,$strDetailType));
		$orderId = intval($arrOrderDetailInfo['order_id']);
		if($orderId > 0){
			$returnArray =  db_factory::get_one(sprintf("select order_id,order_name,order_amount from ".TB_PRE."witkey_order where order_id ='%d' limit 1",$orderId));
		}
		return $returnArray;
		}
	}
	...
	public static function createPayitemRecord($item,$num,$type,$objId){
		if(TOOL === TRUE){
		if($type=='task'){
			$objInfo = db_factory::get_one ( sprintf ( "select * from %switkey_task where task_id='%d'", TB_PRE, $objId ) );
		}else{
			$objInfo = db_factory::get_one ( sprintf ( "select * from %switkey_service where service_id='%d'", TB_PRE, $objId ) );
		}
		$uid = $objInfo['uid'];
		...
		if($intFinaId){
			$objRecord = new Yw_witkey_payitem_record_class ();
			...
			if($item=='tasktop'||$item=='goodstop'){
				...
				if($arrRecordInfo){
				    ...
				    $objRecord->edit_yw_witkey_payitem_record ();
				}else{
					$objRecord->setEnd_time ( time()+24*3600*$num );
					$objRecord->create_yw_witkey_payitem_record ();
				}
			}else{
				 $res = $objRecord->create_yw_witkey_payitem_record();
			}
	...
	public static function setTaskWorkHide($taskId){
        ...
        if(in_array($arrTaskInfo['model_id'],array(4,5))){
        	$strTable = TB_PRE.'witkey_task_bid';
        }else{
        	$strTable = TB_PRE.'witkey_task_work';
        }
	...
	public static function updateObjStatus($item,$type,$objId){
		...
		$arrObjInfo = self::getObjInfo($type,$objId);
		if ($type == 'task') {
			$strTable = TB_PRE . 'witkey_task';
			$strPrimary = ' task_id ';
		} else {
			$strTable = TB_PRE . 'witkey_service';
    ...
    public static function getPayitemConfig( $item_code = null,$pk=null) {
    	...
    	if(TOOL === TRUE){
		$pk or $pk = "item_code";
		$payitem_list = kekezu::get_table_data ( "*", TB_PRE . "witkey_payitem", "1=1 $where ", "", "", "", $pk,3600 );
		if ($item_code) {
	...
	public static function refundPayitem($objId,$objType){
		...
		$payitemConfig = self::getPayitemConfig();
		$strDetailType = "'".implode("','", array_keys($payitemConfig))."'";
		$arrOrderDetailLists = db_factory::query(sprintf("select * from ".TB_PRE."witkey_order_detail where obj_id =%d and obj_type = '%s' and detail_type in(%s) ",$objId,$objType,$strDetailType));
		$orderId = intval($arrOrderDetailLists[0]['order_id']);
		if($orderId > 0){
			$arrOrderInfo =  db_factory::get_one(sprintf("select order_id,order_uid from ".TB_PRE."witkey_order where order_id ='%d' and order_status = 'ok' limit 1",$orderId));
		}
		if($arrOrderInfo){
			foreach ($arrOrderDetailLists as $k=>$v){
				...
				db_factory::execute("update ".TB_PRE."witkey_order set order_status = 'refunded' where order_id = '".$orderId."'");
				if($objType == 'goods'){
					db_factory::execute("update ".TB_PRE."witkey_service set goodstop = '0' where service_id = '".$objId."'");
				}else{
					db_factory::execute("update ".TB_PRE."witkey_task set ".$v['detail_type']." = '0' where task_id = '".$objId."'");
				}
			}
		}
		}
	...
	public static function updateTopitem($objId,$objType){
		if(TOOL === TRUE){
		$sql = "select * from ".TB_PRE."witkey_order_detail where obj_id = '".$objId."' and obj_type = '".$objType."' and detail_type =  '".$objType."top'";
		$payitemRecordInfo = db_factory::get_one($sql);
		if($payitemRecordInfo){
			$topTime = intval($payitemRecordInfo['num']) * 86400 +time();
			db_factory::execute("update ".TB_PRE."witkey_payitem_record set on_time = ".time().",end_time = ".$topTime." where obj_id = '".$objId."' and obj_type = '".$objType."' and item_code =  '".$objType."top'");
		}
	}
	}
}