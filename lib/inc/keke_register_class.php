...
class keke_register_class {
	...
	function __construct($reg_type = 1) {
		...
		$this->_member_obj = new Keke_member_class ();
	...
	function register_login($userinfo,$is_oauth=null) {
		...
		if ($userinfo ['status'] == 3&&$this->_reg_type==1) {
			...
			if($this->_reg_type==2){
					 ...
					 $r = $userinfo;
					db_factory::execute(sprintf(" update yw_company set status=1 where userid='%d'",$userinfo['uid']));
			}
	...
	function save_userinfo($reg_username, $reg_email, $reg_uid = null) {
		...
		$reg_member_uid = $this->_member_obj->create_keke_member ();
		...
		if ($reg_member_uid) {
			...
			$space_id = $this->_space_obj->create_yw_company ();
		}
	...
	function check_ip() {
		...
		$res = $this->_space_obj->query_yw_company ();
		if ($res) {
	...
	public static function register_binding($oauth_user_info, $user_info, $type) {
		...
		$c = db_factory::get_one ( sprintf ( $csql, TB_PRE, $type, $oauth_user_info ['account'] ) );
		if (intval ( $c ['c'] ) == 0) {
			...
			$oauth_obj->setUid ( $user_info ['userid'] );
			$oauth_obj->setUsername ( $user_info ['username'] );
			$oauth_obj->setOn_time ( time () );
			$oauth_obj->create_keke_witkey_member_oauth () or kekezu::show_msg($_lang ['bind_fail'],2);
		} else {
			kekezu::show_msg($_lang ['this_user_has_bind'],7);
		}
		return true;
	}
	public static function is_oauth_bind($type, $oauth_id) {
		$sql = "select * from %switkey_member_oauth where source = '%s' and  oauth_id = '%s'";
		return db_factory::get_one ( sprintf ( $sql, TB_PRE, $type, $oauth_id ) );
	}
