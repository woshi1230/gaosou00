...
class keke_user_login_class {
	...
	function __construct() {
		global $kekezu;
		$this->_space_obj = new Keke_company_class ();
	...
	function get_login_times($uid){
		$rs = db_factory::get_one(sprintf('select `login_times`,`expire`,`on_time` from %switkey_member_black where `uid`=%d',TB_PRE,$this->_login_uid));
		return $rs;
	}
	...
	function login_freeze(){
		if($this->check_black()){
			...
			$db_times!=0 and db_factory::execute(sprintf('update %switkey_member_black set `login_times`=`login_times`-1 where `uid`= %d',TB_PRE,$this->_login_uid));
		}else{
			 $expire =time()+24*3600;
			 db_factory::execute(sprintf('insert into %switkey_member_black (`b_id`,`uid`,`expire`,`on_time`,`login_times`) values(NULL,%d,%d,%d,4)',TB_PRE,$this->_login_uid,$expire,time()));
		}
		$db_times = $this->get_login_times($this->_login_uid);
		return $db_times['login_times'];
	}
	function check_black(){
		$rs = db_factory::get_one(sprintf('select `uid` from %switkey_member_black where `uid`=%d ',TB_PRE,$this->_login_uid));
		if($rs['uid']){
			return true;
		}else {
			return false;
	...
	function valid_moble_auth() {
		...
		$sql =  "
					SELECT
						a.*
					FROM
						`".TB_PRE."witkey_auth_mobile` AS a
					LEFT JOIN `".TB_PRE."witkey_auth_record` AS b ON a.uid = b.uid
	...
	function user_intergration($username, $pwd) {
		...
		if ($u == - 2) {
			...
			$exists = db_factory::get_count ( sprintf ( " select userid from yw_member where userid='%d' ", $u ['uid'] ) );
			if (! $exists) { 
	...
	public function save_user_info($user_info,$account, $ckb_cookie = 1, $login_type = 0,$auto_login=0, $oauth_login = 0) {
		...
		if ($auto_login == '1'){
			$c = array();
			$c[0]= base64_encode($user_info ['userid']);
			$c[1]= base64_encode($account);
			$c[2] =base64_encode($user_info ['userid'].'|'.$_SERVER["HTTP_USER_AGENT"].'|'.$account);
            $token = serialize($c);
			setcookie('keke_auto_login', $token, time()+3600*24*30);
			db_factory::execute("update yw_company set token = '".$token."' where userid = '".$user_info ['userid']."'");
		}
		if ($_K ['refer']) {
			$r = $_K ['refer'];
		} else {
		...
		if ($this->_sys_config ['user_intergration'] ==2) { 
			$synhtml = keke_user_class::user_synlogin ( $user_info ['userid'], $this->_password );
		}
		$synhtml = isset($synhtml)?$synhtml:"";
		$user_obj = new Keke_company_class ();
		$user_obj->setLast_login_time ( time () );
		$user_obj->setWhere ( "userid = '{$user_info['userid']}'" );
		$user_obj->edit_yw_company ();
		$black_obj = new Keke_witkey_member_black_class();
		$black_obj->setWhere("uid = '{$user_info['uid']}'");
		$black_obj->del_keke_witkey_member_black();
		db_factory::execute ( sprintf ( "update %switkey_member_oltime set last_op_time=%d where uid = %d", TB_PRE, time (), $user_info ['userid'] ) );
		if (isset($_COOKIE ['prom'])&&$_COOKIE ['prom']) {
			$prom_obj =  keke_prom_class::get_instance ();
			$url_data = $prom_obj->extract_prom_cookie ();
			$url_data ['p'] == 'reg' or $prom_obj->create_prom_relation ( $user_info ['userid'], $user_info ['username'], $url_data, '2' );
		}
		if ($login_type == 1) {
			...
			$return_info ['pic'] = keke_user_class::get_user_pic ( $user_info ['userid'] );
			$return_info ['syn'] = $synhtml;
			($user_info ['userid'] == ADMIN_UID || $user_info ['groupid'] > 0) and $return_info ['is_admin'] = 1;
