...
class keke_tpl_class {
	...
	static function parse_template($tpl) {
		...
		$objfile = S_ROOT . '/file/cache/tpl/' . str_replace ( '/', '_', $tpl ) . '.php';
	...
	public static function parse_rule($template, $tpl = null) {
		global $_K;
		($_K ['inajax']) && ob_start ();
//		keke_tpl_class::readtemplate('k_header');
	...
	public static function template_parse($str) {
		...
		$str = preg_replace("/<\?php([^\?]+)\?>/es", "keke_tpl_class::template_addquote('<?php\\1?>')", $str);
		$str = preg_replace("/\{(\\$[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\+\-\x7f-\xff]*)\}/", "<?php echo \\1;?>", $str);
		$str = preg_replace("/\{(\\$[a-zA-Z0-9_\[\]\'\"\$\+\-\x7f-\xff]+)\}/es", "keke_tpl_class::template_addquote('<?php echo \\1;?>')", $str);
	...
	static function template_addquote($var) {
	...
	static function tpl_exists($tplname) {
		global $_K;
		is_file ( S_ROOT . "template/default/" . $tplname . ".htm" ) and $tpl = "template/default/$tplname" or $tpl = $tplname;
		return $tpl;
	}
