...
abstract class keke_report_class {
	...
	public static function get_report_info($report_id) {
		$sql = sprintf ( "select * from %switkey_report where report_id='%d'", TB_PRE, $report_id );
		return db_factory::get_one ( $sql );
	}
	public static function get_task_info($task_id) {
		$sql = sprintf ( "select * from %switkey_task where task_id='%d'", TB_PRE, $task_id );
		return db_factory::get_one ( $sql );
	}
	public static function get_service_info($service_id) {
		$sql = sprintf ( "select * from %switkey_service where service_id='%d'", TB_PRE, $service_id );
		return db_factory::get_one ( $sql );
	}
	public static function get_model_info($model_id) {
//		$sql = sprintf ( "select model_code from %switkey_model where model_id = '%d' ", TB_PRE, $model_id );
//		return db_factory::get_one ( $sql );
		return Keke_witkey_model_class::get_one("*", $model_id);
	}
	public static function check_if_report($type, $obj, $obj_id, $uid, $to_uid) {
		...
		$reportSql = sprintf ( " select report_id from %switkey_report where report_type='%d' and obj='%s'
		 and obj_id='%d' and uid='%d' and to_uid='%d'", TB_PRE, $type, $obj, $obj_id, $uid, $to_uid );
	...
	public static function obj_info_init($report_info, $user_info) {
		global $kekezu, $_K, $REF;
		global $_lang;
		switch ($report_info ['obj']) { 
			case "task" : 
				$obj_info = db_factory::get_one ( sprintf ( " select task_id origin_id,uid origin_uid,model_id,task_title origin_title,task_status as origin_status,real_cash as cash from %switkey_task where task_id='%d'", TB_PRE, $report_info ['origin_id'] ) );
				$re_obj = "<a href=\"" . $_K ['siteurl'] . "/index.php?do=task&id=" . $obj_info ['origin_id'] . "\">" . $obj_info ['origin_title'] . "</a>";
				break;
			case "work" : 
				$model_id = db_factory::get_count ( sprintf ( " select model_id from %switkey_task where task_id='%d'", TB_PRE, $report_info ['origin_id'] ) );
...
//				$model_id or kekezu::admin_show_msg ( $_lang ['friendly_notice'], 'index.php?do=trans&view=' . $view, 2, $_lang ['this_task_has_delete'] );
				$model_id or kekezu::admin_show_msg ( $_lang ['friendly_notice'], "{$REF['scheme']}://{$REF['host']}{$REF['path']}?{$REF['query']}", 2, $_lang ['this_task_has_delete'] );
				...
				if ($model_info ['model_code'] == 'dtender' || $model_info ['model_code'] == 'tender') { 
					...
					$bid_count = db_factory::get_count ( sprintf ( " select count(bid_id) from %switkey_task_bid where task_id='%d' and bid_status='4'", TB_PRE, $report_info ['origin_id'] ) );
				} else { 
					...
					$bid_count = db_factory::get_count ( sprintf ( " select count(work_id) from %switkey_task_work where task_id='%d' and work_status not in(0,5,7,8) ", TB_PRE, $report_info ['origin_id'] ) );
				}
				$obj_info = db_factory::get_one ( sprintf ( $sql, TB_PRE, TB_PRE, $report_info ['obj_id'] ) );
				...
				break;
			case "product" : 
				$obj_info = db_factory::get_one ( sprintf ( " select service_id origin_id,uid origin_uid,model_id,title origin_title from %switkey_service where service_id='%d'", TB_PRE, $report_info ['obj_id'] ) );
	...
	public static function accept_notify($report_info, $user_info, $re_obj) {
		...
		keke_msg_class::notify_user ( $user_info ['userid'], $user_info ['username'], 'transrights_accept', $trans_name . $_lang ['accepted_notice'], $result, 1 );
	}
	public static function process_notify($process_type = 'pass', $report_info, $user_info, $to_userinfo, $process_result, $process_reply = '') {
		...
		if ($report_info ['report_type'] != '1') {
			...
			if ($process_type == 'pass') { 
				...
				keke_msg_class::notify_user ( $user_info ['userid'], $user_info ['username'], 'transrights_pass', $trans_name . $_lang ['process_notice'], $result, 1 );
			...
			if ($process_type == 'nopass') { 
				...
				keke_msg_class::notify_user ( $user_info ['userid'], $user_info ['username'], 'transrights_nopass', $trans_name . $_lang ['process_notice'], $result, 1 );
		...
		if ($report_type == '1') {
			...
			if ($report_satus == '1') { 
				switch ($obj) {
					case "task" :
					case "work" :
						$task_info = db_factory::get_one ( sprintf ( " select task_title from %switkey_task where task_id='%d'", TB_PRE, $origin_id ) );
						$res = db_factory::execute ( sprintf ( " update %switkey_task set task_status='11' where task_id='%d' ", TB_PRE, $origin_id ) );
						$re_type = $_lang ['task'];
						$re_obj = "<a href=\"" . $_K ['siteurl'] . "/index.php?do=task&id=" . $origin_id . "\">" . $task_info ['task_title'] . "</a>";
						break;
					case "order" :
						$order_info = db_factory::get_one ( sprintf ( " select order_name origin_title, model_id from %switkey_order where order_id='%d'", TB_PRE, $obj_id ) );
						$res = db_factory::execute ( sprintf ( " update %switkey_order set order_status='arbitral' where order_id='%d' ", TB_PRE, $obj_id ) );
						$re_type = $_lang ['order'];
						if ($user_info ['userid'] == $order_info ['seller_uid']) {
				...
				if ($res) { 
					...
					keke_msg_class::notify_user ( $user_info ['userid'], $user_info ['username'], 'transrights_freeze', $re_type . $_lang ['freeze_notice'], $result, 1 );
					keke_msg_class::notify_user ( $to_user_info ['userid'], $to_user_info ['username'], 'transrights_accept', $re_type . $_lang ['freeze_notice'], $result2, 1 );
				}
			}
		}
	}
	public function disablePeople() {
		$sql = sprintf ( "update yw_company set groupid=2 where userid =%d", $this->_to_user_info ['userid'] );
		db_factory::execute ( $sql );
		$sql = sprintf ( "update yw_member set groupid=2 where userid =%d", $this->_to_user_info ['userid'] );
		db_factory::execute ( $sql );
	}
			...
			if ($report_type == '1') { 
				switch ($report_info ['obj']) {
					...
					case "work" : 
						if ($process_type == 'pass') {
							$to_status = '9';
							$res = db_factory::execute ( sprintf ( " update %switkey_task set task_status='%d' where task_id='%d' ", TB_PRE, $to_status, $report_info ['origin_id'] ) );
						} else { 
							$res = db_factory::execute ( sprintf ( " update %switkey_task set task_status='%d' where task_id='%d' ", TB_PRE, $front_status, $report_info ['origin_id'] ) );
						}
						break;
					case "order" : 
						if ($process_type == 'pass') {
							$res = db_factory::execute ( sprintf ( " update %switkey_order set order_status='close' where order_id='%d' ", TB_PRE, $report_info ['obj_id'] ) );
						} else { 
							$res = db_factory::execute ( sprintf ( " update %switkey_order set order_status='%s' where order_id='%d' ", TB_PRE, $front_status, $report_info ['obj_id'] ) );
						}
						break;
					case "product" :
						break;
	...
	public static function change_status($report_id, $status, $op_result = null, $process_result = null) {
		$sql = sprintf ( "update %switkey_report set report_status= '%d',op_uid='%d',op_username='%s',op_result='%s',op_time='%d'  where report_id='%d'", TB_PRE, $status, $op_result ['op_uid'], $op_result ['op_username'], $process_result, $op_result ['op_time'], $report_id );
		return db_factory::execute ( $sql );
	}
	public function reset_task($task_id, $delay_days) {
		$end_time = time () + $delay_days * 3600 * 24;
		$sql = sprintf ( "update %switkey_task set task_status = 2,end_time = %d where task_id = %d ", TB_PRE, $end_time, $task_id );
		return db_factory::execute ( $sql );
	}
	public function reset_agreement($task_id) {
		$sql = sprintf ( "update %switkey_agreement set agree_status = 1,buyer_status = 1,seller_status = 1 where task_id = %d ", TB_PRE, $task_id );
		return db_factory::execute ( $sql );
	}
	public function shield_work($obj_id) {
		...
		if ($model_info ['model_code'] == 'dtender' || $model_info ['model_code'] == 'tender') { 
			return db_factory::execute ( sprintf ( "update %switkey_task_bid set bid_status = 8 where bid_id = '%d'", TB_PRE, $obj_id ) );
		} else {
			return db_factory::execute ( sprintf ( "update %switkey_task_work set work_status = 8 where work_id = '%d'", TB_PRE, $obj_id ) );
		}
	}
	public function cancel_bid($obj_id) {
		...
		$model_info = $kekezu->_model_list [$this->_obj_info ['model_id']];
		if ($model_info ['model_code'] == 'dtender' || $model_info ['model_code'] == 'tender') { 
			db_factory::execute ( sprintf ( "update %switkey_task_bid set bid_status = 8 where bid_id = '%d'", TB_PRE, $obj_id ) );
		} else {
			db_factory::execute ( sprintf ( "update %switkey_task_work set work_status = 8 where work_id = '%d'", TB_PRE, $obj_id ) );
		}
		db_factory::execute ( sprintf ( " update yw_company set accepted_num = accepted_num-1 where userid = '%d'", $this->_obj_info ['obj_uid'] ) );
	...
	public static function get_report_employee_reason() {
		return array (
				'sxcx' => '高手（卖家）涉嫌抄袭',
				'waswcgz' => '高手（卖家）未按时完成工作',
				'jjxgzp' => '高手（卖家）拒绝修改作品',
				'yqzjsj' => '高手（卖家）要求追加赏金',
				'wnlwcyq' => '高手（卖家）无能力完成要求',
				'qt' => '高手（卖家）其它'
		);
	}
	...
	function less_credit($value, $type) {
		...
		if ($type == 1) {
			$sql = sprintf ( "update yw_company set buyer_credit = buyer_credit-%d where userid=%d", $value, $uid );
			$b_level = keke_user_mark_class::get_mark_level ( $user_info ['buyer_credit'], 2 );
			$up_sql = " UPDATE yw_company set buyer_level='" . serialize ( $b_level ) . "' where userid=" . $uid;
		} elseif ($type == 2) {
			$sql = sprintf ( "update yw_company set seller_credit = seller_credit-%d where userid=%d", $value, $uid );
			$s_level = keke_user_mark_class::get_mark_level ( $user_info ['seller_credit'], 1 );
			$up_sql = " UPDATE yw_company set seller_level='" . serialize ( $s_level ) . "' where userid=" . $uid;
		}
	...
	function to_black($days) {
		...
		if ($days) {
			$sql = sprintf ( "update yw_company set status=2 where userid=%d", $uid );
			db_factory::execute ( $sql );
		} else {
			return false;
		}
		$expire = $days * 3600 * 24;
		$sql2 = sprintf ( "select count(*) as c  from %switkey_member_black where uid = %d ", TB_PRE, $uid );
		if (db_factory::get_count ( $sql2 )) {
			$sql3 = sprintf ( "update %switkey_member_black set expire = if(expire>unix_timestamp(),expire+%d, unix_timestamp()+%d) where uid = %d ", TB_PRE, $expire, $expire, $uid );
		} else {
			$sql3 = sprintf ( "insert into %switkey_member_black (uid,expire,on_time) values (%d,%d,%d)", TB_PRE, $uid, $expire, time () );
		}
		db_factory::execute ( $sql3 );
		return true;
	}
	public static function getFrontStatus($originId) {
		$arrReportInfo = db_factory::get_one ( "select * from " . TB_PRE . "witkey_report where origin_id=" . $originId . "
