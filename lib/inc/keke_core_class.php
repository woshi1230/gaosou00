...
class keke_core_class extends keke_base_class {
	...
	static function admin_show_msg($title = "", $url = "", $time = 3, $content = "", $type = "info") {
		global $_K, $_lang;
		$url ? $url : $_K ['refer'];
		require keke_tpl_class::template (ADMIN_DIRECTORY.'/template/show_msg' );
		die ();
	}
	static function show_msg($title = "", $url = "", $time = 3, $content = "", $type = 'info') {
		global $_K, $basic_config, $username, $uid, $_lang, $strWebLogo, $indus_arr, $arrTopIndus, $indus_goods_arr, $indus_task_arr;
		...
		unset ( $json, $_K, $basic_config, $username, $uid, $_lang, $strWebLogo, $indus_arr, $arrTopIndus, $indus_goods_arr, $indus_task_arr );
		die ();
	}
	...
	static function get_cash_consume($total) {
		...
		$tmp = array ();
		$credit_allow = '2';//$kekezu->_sys_config ['credit_is_allow'];
	...
	public static function set_favor($pk, $keep_type, $model_code, $obj_uid, $obj_id, $obj_name, $origin_id, $url = '', $output = 'normal') {
		...
		$favor_obj = new Keke_favorite_class ();
		...
		$f_id = $favor_obj->create_keke_favorite ();
		if ($f_id) {
			if (in_array ( $keep_type, array (
				...
				$up_tab = TB_PRE . "witkey_" . $keep_type;
	...
	public static function check_if_favor($uid, $obj_uid, $pk, $keep_type, $model_code, $obj_id, $url = '', $output = 'normal') {
		global $_lang;
		$favor_type = keke_glob_class::get_favor_type ();
		$favor_tab = TB_PRE . "witkey_" . $keep_type;
		if ($obj_uid == $uid) {
			kekezu::keke_show_msg ( $url, $_lang ['you_can_not_collection_self'] . $favor_type [$keep_type] . "！", "error", $output );
		} else {
			$if_favor = db_factory::get_count ( sprintf ( " select itemid from %sfavorite where keep_type='%s' and typeid='%d' and obj_id='%d' and userid='%d'", TB_PRE, $keep_type, $model_code, $obj_id, $uid ) );
			if (! $if_favor) {
				return TRUE;
			} else {
	...
	static function send_mail($address, $title, $body) {
		...
		$mail->MsgHTML ( $body );
		$address_email = db_factory::get_one ( 'select username from yw_company where email="' . $address . '"' );
	...
	static function get_rand_kf() {
		$kf_arr = kekezu::get_table_data ( '*', 'yw_company', ' groupid = 7', '', '', '', '', null );
	...
	static function check_user_by_name($user, $isusername = 0) {
		...
		$user_count = $member_obj->count_keke_member ();
		return $user_count;
	}
	...
	static function save_feed($feed_arr, $uid, $username, $feedtype = "", $obj_id = 0, $obj_link = "", $icon = ''){
		...
		return db_factory::inserttable ( TB_PRE . 'witkey_feed', $insertsqlarr );
	}
	static function get_following_count($uid) {
		$c = db_factory::execute ( sprintf ( "select follow_id from %switkey_free_follow where uid = '%d'", TB_PRE, $uid ) );
		return $c;
	}
	static function get_follower_count($uid) {
		$c = db_factory::execute ( sprintf ( "select follow_id from %switkey_free_follow where fuid = '%d'", TB_PRE, $uid ) );
		return $c;
	}
	static function get_if_focus($uid, $fuid) {
		$c = db_factory::get_one ( sprintf ( "select follow_id from %switkey_free_follow where uid = '%d' and fuid = '%d'", TB_PRE, $uid, $fuid ) );
		return $c;
	}
	static function get_feed($where_arr, $order, $limit) {
		$feed_arr = kekezu::get_table_data ( "*", TB_PRE . "witkey_feed", $where_arr, $order, "", $limit, "feed_id" );
		$feed_new_arr = array ();
	...
	static function notify_user($title, $content, $uid, $username = "") {
		...
		if (is_array ( $content )) {
		    ...
		    $res = $objMsgConfig->query_yw_message_config();
	...
	public static function autoload($class_name) {
		try {
			...
			if (! $i_model && isset ( $kekezu->_model_list )) {
				...
				$auth_item = self::get_table_data ( 'auth_code,auth_dir', TB_PRE . 'witkey_auth_item', '', 'listorder asc ', '', '', 'auth_code', null );
...
//			$where = " where model_id= '$model_id' ";
			$model_config=Keke_witkey_model_class::get_one("*", $model_id);
		} else {
			$model_config=Keke_witkey_model_class::query("*");
		}
//		$model_config = db_factory::query ( ' select model_id,config from ' . TB_PRE . "witkey_model $where ", true, 60 * 20 );
		if ($model_id) {
...
//		if (! $pay_item) {
//			$pay_item = array ();
//			$item = db_factory::query ( " select a.*,b.model_id from " . TB_PRE . "witkey_pay_item as a left join " . TB_PRE . "witkey_model as b on a.model_code =b.model_dir" );
//			foreach ( $item as $v ) {
...
//		return $pay_item;
	...
	public static function get_payment_config($paymentname = "", $pay_type = 'online', $pay_status = null) {
		if ($paymentname) {
			...
			$list = kekezu::get_table_data ( '*', TB_PRE . "witkey_pay_api", $where, "", '', '', '', null );
			if ($list) {
			...
			if ($pay_type == 'offline') {
				$list = kekezu::get_table_data ( 'payment', TB_PRE . "witkey_pay_api", " type='offline'", '', '', '', '', null );
				$i = 0;
	...
	public static function get_config_rule($ruletype, $nokey = '') {
		global $kekezu;
		return kekezu::get_table_data ( "*", TB_PRE . $ruletype, "", "", "", "", "", $nokey, null );
	}
	public static function get_industry($pid = NULL, $cache = NULL) {
		! is_null ( $pid ) and $where = " parentid = '" . intval ( $pid ) . "'";
		$indus_arr = self::get_table_data ( '*', TB_PRE . "category", $where, "listorder", '', '', 'catid', $cache );
		return $indus_arr;
	}
	public static function get_indus_info($indus_id) {
		$indus_id and $indus_info = db_factory::get_one ( sprintf ( "select * from %scategory where catid=%d", TB_PRE, $indus_id ) );
		return $indus_info;
	}
	...
	public static function get_cash_cove($model_code = 'tender', $all = false) {
		...
		return self::get_table_data ( '*', TB_PRE . "witkey_task_cash_cove", $w, "start_cove", '', '', 'cash_rule_id', null );
	}
	...
	static function get_ad($adname = null, $limit_num = null) {
		...
		return self::get_table_data ( '*', TB_PRE . 'witkey_ad', '1=1 and is_allow=1 ' . $where, 'listorder', '', $limit, '', 3600 );
	}
	...
	static function update_oltime() {
		...
		$user_oltime = db_factory::get_one ( sprintf ( "select last_op_time from %switkey_member_oltime where uid = '%d'", TB_PRE, $login_uid ) );
		if ((SYS_START_TIME - $user_oltime ['last_op_time']) > $_K ['timespan']) {
			$res = db_factory::execute ( sprintf ( "update %switkey_member_oltime set online_total_time = online_total_time+%d,last_op_time = '%d' where uid = '%d'", TB_PRE, $_K ['timespan'], SYS_START_TIME, $login_uid ) );
		}
		return $res;
	}
	static function favorableRate($uid,$role){
		...
		$info = db_factory::get_one("SELECT buyer_good_num,buyer_total_num,seller_good_num,seller_total_num FROM yw_company WHERE userid = ".intval($uid));
		if($info){
			if($role == '2'){
				$rate = 0;
	...
	static function get_user_online($uid) {
		$user_oltime = db_factory::get_one ( sprintf ( "select last_op_time from %switkey_member_oltime where uid = '%d'", TB_PRE, $uid ) );
	...
	static function getmessagecount($uid){
		$intCountTrends = db_factory::get_count("select count(itemid) from ".TB_PRE."message where fromuid<1 and isread!=1 and typeid=1 and  touid=".intval($uid));
		$intCountNotice = db_factory::get_count("select count(itemid) from ".TB_PRE."message where fromuid<1 and isread!=1 and typeid=2 and  touid=".intval($uid));
		$intCountPrivate = db_factory::get_count("select count(itemid) from ".TB_PRE."message where touid = ".intval($uid)." and isread!=1 and fromuid>0 and status<>2 ");
...
//			$this->init_session ();
	...
	function init_config() {
		global $i_model, $_lang, $_K, $CFG;
//		$this->_basic_arr = $basic_arr = db_factory::query ( 'select config_id,k,v,type,listorder from ' . TB_PRE . 'witkey_basic_config' );
		$this->_basic_arr = $basic_arr = Keke_witkey_basic_config_class::query ( 'select config_id,k,v,type,listorder from witkey_basic_config' );
		$config_arr = array ();
        foreach ($basic_arr as $v){
            $config_arr [$v ['k']] = trim($v ['v']);
            if($v['k'] == 'ban_content' || $v['k'] == 'ban_users'){
                $config_arr [$v['k']] = trim(trim($v['v']),'|');
            }
        }

		define('DT_CACHE', $CFG['cache_dir'] ? $CFG['cache_dir'] : DT_ROOT.'/file/cache');
		$CACHE = $this->cache_read('module.php');
		$config_arr ['website_name'] = $CACHE['dt']['sitename'];

		$mtime = explode ( ' ', microtime () );
		$_K ['autoshop'] = '1';//$config_arr['autoshop'];
		$_K ['timestamp'] = $mtime [1];
		$_K ['charset'] = CHARSET;
		$_K ['sitecss'] = 'css-skype';//$config_arr ['sitecss'];
		$_K ['css_auto_fit'] = '0';//$config_arr ['css_auto_fit'];
		$_K ['theme'] = '';//$config_arr ['theme'];
		$_K ['sitename'] = $config_arr ['website_name'];
		$_K ['siteurl'] = $CFG ['url'];
		$_K ['inajax'] = 0;
		$_K ['block_search'] = array ();
		$_K ['is_rewrite'] = '0';//$config_arr ['is_rewrite'];
		$_K ['timespan'] = '600';
		$_K ['i'] = 0;
	...
	function init_user() {
		if ($_SESSION ['uid']) {
			...
			if (! $userinfo ['last_login_time']) { 
				db_factory::execute ( ' update yw_member set logintime=' . time () . ' where userid=' . $this->_uid );
			...
			$uInfo = kekezu::get_table_data ( '*', 'yw_company', " username='$pwdInfo[2]' and token = '$token'", '', '' );
...
//		$this->_indus_p_arr = kekezu::get_table_data ( '*', TB_PRE . "witkey_industry", "indus_type=1 and indus_pid = 0 ", " CASE WHEN listorder = 0 THEN 9999999 WHEN listorder > 0 THEN listorder END ", '', '', 'indus_id', NULL );
//		$tmpArrIndus= kekezu::get_table_data ( '*', TB_PRE . 'witkey_industry', 'indus_type=1 and indus_pid >0', ' CASE WHEN listorder = 0 THEN 9999999 WHEN listorder > 0 THEN listorder END ', '', '', 'indus_id', NULL );
...
//			}
//		}
		$this->_indus_arr = kekezu::get_table_data ( '*', TB_PRE . 'category', 'moduleid=42 and parentid>0', 'listorder', '', '', 'catid', NULL );
//		$arrtotask = db_factory::query("show COLUMNS FROM ".TB_PRE.'witkey_industry'." WHERE Field='totask' ");
//		if($arrtotask[0]){
//			$this->_indus_task_arr = kekezu::get_table_data ( '*', TB_PRE . "witkey_industry", "indus_type=1 and indus_pid = 0 and totask=1 ", " CASE WHEN listorder = 0 THEN 9999999 WHEN listorder > 0 THEN listorder END ", '', '', 'indus_id', NULL );
//		}
//		$arrtogoods = db_factory::query("show COLUMNS FROM ".TB_PRE.'witkey_industry'." WHERE Field='togoods' ");
//		if($arrtogoods[0]){
//			$this->_indus_goods_arr = kekezu::get_table_data ( '*', TB_PRE . "witkey_industry", "indus_type=1 and indus_pid = 0 and togoods=1 ", " CASE WHEN listorder = 0 THEN 9999999 WHEN listorder > 0 THEN listorder END ", '', '', 'indus_id', NULL );
//		}
		$this->_indus_task_arr = kekezu::get_table_data ( '*', TB_PRE . "category", "moduleid=42 and parentid=0", "listorder", '', '', 'catid', NULL );
	}
	function init_oauth() {
...
//		}
	...
	function init_model() {
//		$model_arr = db_factory::query ( 'select * from ' . TB_PRE . 'witkey_model where 1=1 order by  model_id asc', 0, null );
		$model_arr = Keke_witkey_model_class::query("*");
//		$this->_model_list = kekezu::get_arr_by_key ( $model_arr, 'model_id' );
		$this->_model_list = $model_arr;
