...
class keke_loaddata_class {
	...
	static function readtag($name) {
		global $kekezu,$_lang;
		$arrTag = db_factory::get_one(sprintf('select * from %s where tagname = "%s"',TB_PRE.'witkey_tag',$name));
	...
	static function ad_show($code, $do = 'index',$is_slide=null) {
		global $_lang,$_K;
		$ad_target = db_factory::get_one ( sprintf ( " select * from %switkey_ad_target where code='%s' and is_allow=1", TB_PRE, $code ),3600*24);
			if ($ad_target) {
			if(intval($is_slide)){
			...
			$ad_info = db_factory::get_one( sprintf ( $sql, TB_PRE, TB_PRE, $code, $ad_target ['ad_num'] ),true,$_K['timespan'] );
			if($ad_info){
	...
	static function update_ad($ad_info){
	   global $_K,$kekezu;
	     if((SYS_START_TIME - intval($ad_info['on_time']))>$_K['timespan']){
	      db_factory::execute(sprintf('update %switkey_ad set on_time  = %s where ad_id = %d',TB_PRE,time()+$_K['timespan'],$ad_info['ad_id']));
	      db_factory::execute(sprintf("update %switkey_ad set is_allow='0' where ad_id='%d' and  end_time>0 and end_time<%d",TB_PRE,$ad_info['ad_id'],time()));
	   }
	}
	...
	static function get_adgroup_by_target($target_id,$target_name,$ad_list_num){
		global $kekezu,$_K;
		$arrTag = db_factory::get_one(sprintf('select * from %s where target_id = %d',TB_PRE.'witkey_ad_target',intval($target_id)));
		$datalist = kekezu::get_table_data ( '*', TB_PRE . 'witkey_ad', '1=1 and is_allow=1 and target_id = '.intval($target_id), 'listorder', '', $ad_list_num, '', 3600 );
