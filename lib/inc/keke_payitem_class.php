...
class keke_payitem_class {
	...
	public static function get_payitem_record($w = array(), $order = null, $p = array()) {
		global $kekezu;
		$record_obj = new Yw_witkey_payitem_record_class ();
		$record_arr = array ();
		$where = " 1 = 1 ";
		if (! empty ( $w )) {
		...
		if (! empty ( $p )) { 
			...
			$count = intval ( $record_obj->count_yw_witkey_payitem_record () );
			$pages = $page_obj->getPages ( $count, $page_size, $page, $url, "#" . $anchor );
			$where .= $pages ['where'];
		}
		$record_obj->setWhere ( $where );
		$record_list = $record_obj->query_yw_witkey_payitem_record ();
	...
	public static function payitem_cost($item_code, $use_num = '1', $obj_type = false, $use_type = 'buy', $obj_id = null, $origin_id = null) {
		...
		$record_id = $record_obj->create_yw_witkey_payitem_record ();
		return $record_id;
	}
	public static function payitem_del($record_id) {
		return db_factory::execute ( sprintf ( " delete frm %switkey_payitem_record where record_id='%d'", TB_PRE, $record_id ) );
	}
	...
	public static function payitem_exists($uid, $item_code=false, $obj_type=false,$payitem_arr=false) {
		if($payitem_arr){ 
			foreach ($payitem_arr as $k=>$v) { 
				$buy_count = db_factory::get_count ( sprintf ( " select sum(use_num) from %switkey_payitem_record where uid = '%d'  and item_code = '%s' and use_type = 'buy'", TB_PRE, $uid,  $v[item_code] ) );
				$use_count = db_factory::get_count ( sprintf ( " select sum(use_num) from %switkey_payitem_record where uid = '%d'  and item_code = '%s' and use_type = 'spend'", TB_PRE, $uid,  $v[item_code] ) );
				$payitem_info [$v[item_code]]  = intval($buy_count - $use_count); 
			} 
		}else{ 
		$buy_count = db_factory::get_count ( sprintf ( " select sum(use_num) from %switkey_payitem_record where uid = '%d' and item_code = '%s' and use_type = 'buy'", TB_PRE, $uid, $item_code ) );
		$use_count = db_factory::get_count ( sprintf ( " select sum(use_num) from %switkey_payitem_record where uid = '%d'  and item_code = '%s' and use_type = 'spend'", TB_PRE, $uid, $item_code ) );
	...
	public static function get_user_payitem($uid, $use_type = null, $obj_type = null, $obj_id = null) {
		$sql = " select a.use_cash,a.item_code,b.item_name,b.small_pic,b.item_desc from " . TB_PRE . "witkey_payitem_record a left join " . TB_PRE . "witkey_payitem b
	...
	public static function get_payitem_info($user_type,$model_code){
		...
		$payitem_arr = kekezu::get_table_data("*",TB_PRE . "witkey_payitem","$where","","","","item_id");
		return $payitem_arr;
	}
	public static function update_service_payitem_time($payitem_time,$add_time,$service_id){
			...
			$res = db_factory::execute(sprintf("update %switkey_service set payitem_time='%s' where service_id=%d",TB_PRE,$new_payitem_time,$service_id));
			return $res;
	}
	public static function set_payitem_time($payitem_arr,$obj_id,$obj_type){
		...
		switch ($obj_type){
			case "task":
				$sql = sprintf("update %switkey_task set payitem_time='%s' where task_id=%d",TB_PRE,$payitem_end_time,$obj_id);
				break;
			case "service":
				$sql = sprintf("update %switkey_service set payitem_time='%s' where service_id=%d",TB_PRE,$payitem_end_time,$obj_id);
				break; 
		}   
