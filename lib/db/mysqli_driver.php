...
final class mysqli_drver extends DataBase {
	...
	function __construct() {
		global $CFG;
		$this->_dbhost = $CFG['db_host'];
		$this->_dbname = $CFG['db_name'];
		$this->_dbuser = $CFG['db_user'];
		$this->_dbpass = $CFG['db_pass'];
		$this->_dbcharset = $CFG['db_charset'];
	}
	...
	public function select($fileds = '*', $table, $where = '', $order = '', $group = '', $limit = '', $pk = '', $cachetime = 0) {
		...
		$sql = 'SELECT ' . $fileds . ' FROM `' . $this->_dbname . '`.`' . $table . '`' . $where . $group . $order . $limit;
		$this->execute_sql ( $sql );
		$datalist = array ();
		while ( ($rs = $this->fetch_array ()) != false ) {
