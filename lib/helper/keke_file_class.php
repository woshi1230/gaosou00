...
class keke_file_class {
	...
	static function file_down($file_name, $file_path) {
		...
		if($file_name&&$file_path){
			if(strpos($file_path, 'file/upload/') !== false&&strpos($file_path, '../')===false ){
	...
	public static function del_file($filepath, $is_db = true) {
		if ($is_db or db_factory::get_count ( sprintf ( " select count(file_id) from %switkey_file where save_name='%s'", TB_PRE, $filepath ) )) {
			if (is_file ( $filepath )) {
	...
	public static function upload_file($file_name, $ext = '', $isr = 1, $folder = '', $output = 'normal') {
		...
		if ($_FILES [$file_name]['size']) {
			...
			if ($folder != '') {
				$absolute_path = S_ROOT . '/file/upload/sys/' . $folder;
				$filepath = 'file/upload/sys/' . $folder;
			} else {
				$absolute_path = UPLOAD_ROOT;
				$filepath = 'file/upload/' . UPLOAD_RULE;
			}
