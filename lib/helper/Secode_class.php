...
class Secode_class {
	...
	function doImage() {
		...
		$this->createCode ();
		if($this->is_calculate){
			$this->createCalculationVerificationCode();
		}
	...
	function saveData() {
		...
		if($this->is_calculate){
			$_SESSION ['securimage_code_value'] = strtolower ( $this->calculate_result );
		}
	}
	function validate() {
		if (isset ( $_SESSION ['securimage_code_value'] ) && ! empty ( $_SESSION ['securimage_code_value'] )) {
	...
	function generateCalculationResult(){
		$this->code = $this->generateCalculationExpression();
		$result = 0;
		$code = str_replace(array('=','?'), array('',''), $this->code);
		eval ( "\$result = " . $code . ";" );
		$this->calculate_result = $result;
		return  $result;
	}
	function generateCalculationExpression(){
		$operator = '+*'; 
		$code = array ();
		$code [] = mt_rand ( 1, 9 );
	...
	function createCalculationVerificationCode(){
