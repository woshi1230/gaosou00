...
class session_mysql_class {
	...
	function read($session_id) {
		$sql = "select session_data from " . TB_PRE . "witkey_session where session_id = '$session_id' and session_expirse>" . time ();
		$session_arr = $this->_db->get_one( $sql, 1 );
		empty ( $session_arr ) and $session_data = '' or $session_data = $session_arr['session_data'];
		return $session_data;
	}
	function write($session_id, $session_data) {
		$tablename = TB_PRE . "witkey_session";
	...
	function destroy($session_id) {
		$sql = "delete from " . TB_PRE . "witkey_session where session_id ='$session_id' ";
		return db_factory::execute ( $sql );
	}
	function gc($max_left_time) {
		$sql = "delete from " . TB_PRE . "witkey_session where session_expirse <" . time ();
		return db_factory::execute ( $sql );
	}
}
class session_file_class {
	function __construct() {
		global $CFG;
		$path = S_ROOT . 'file' . DIRECTORY_SEPARATOR . 'session'. DIRECTORY_SEPARATOR . strtolower(substr(md5($CFG['authkey']), 2, 6));
