...
$arrArtInfo = db_factory::get_one(sprintf($strSql,TB_PRE,TB_PRE,$intId));
...
$arrArtKeyword = db_factory::query("select * from ".TB_PRE."witkey_article_keyword where keyword_status=1");
if(is_array($arrArtKeyword)){
	foreach ($arrArtKeyword as $v){
		...
		if($intShowCount&&!$arrArtInfo['views']){
			db_factory::execute(sprintf("update %switkey_article_keyword set show_count = show_count +%d where keyword_id=%d",TB_PRE,intval($intShowCount),$v['keyword_id']));
		}
	}
}
$strWhere = " and 1=1 ";
$intCatid and $strWhere .= " and  art_cat_id  = ".intval($arrArtInfo['art_cat_id']);
$arrArtUp = db_factory::get_one(sprintf("select  art_id ,art_cat_id,art_title  from %switkey_article  where cat_type = 'article' and  art_id<'%d'  %s order by art_id desc limit 0,1",TB_PRE,$intId,$strWhere));
$arrArtDown = db_factory::get_one(sprintf("select art_id ,art_cat_id,art_title  from %switkey_article  where cat_type = 'article' and  art_id>'%d' %s order by art_id asc limit 0,1",TB_PRE,$intId,$strWhere));
if(!$_COOKIE["article_".$intId]){
	$sqlplus = "update %switkey_article set views = views+1 where art_id = %d";
	db_factory::execute(sprintf($sqlplus,TB_PRE,$intId));
}
