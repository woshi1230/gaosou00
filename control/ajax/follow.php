...
if($op=='add'){
	...
	$countFollow = db_factory::get_count(sprintf('select count(*) from %s where uid = %d and fuid = %d',TB_PRE.'witkey_free_follow',intval($gUid),intval($id)));
	if($countFollow){
	...
	$followInfo = db_factory::get_one("select * from ".TB_PRE."witkey_free_follow where ".'uid = '.intval($gUid).' and fuid = '.intval($id));
	if(!$followInfo){
		exit('无权操作');
	}
