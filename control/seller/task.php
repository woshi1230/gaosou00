...
if(is_array($arrTaskLists)){
	foreach ($arrTaskLists as $k=>$v) {
		$arrFavorite = db_factory::get_count(sprintf('select count(*) from %s where userid = %d and obj_id = %d and keep_type = "task"',TB_PRE.'favorite',intval($gUid),intval($v['task_id'])));
		if($arrFavorite){
