...
if($type==2){
	...
	$arrFoundCount = kekezu::get_table_data ( " sum(fina_cash) cash,sum(fina_credit) credit,count(fina_id) count,fina_action ", TB_PRE . "witkey_finance", " uid=".$arrSellerInfo['uid']." and fina_action in ('pub_task','task_bid','buy_service','sale_service','sale_gy') ", "", " fina_action ", "", "fina_action" );
	...
	$arrFoundCount = kekezu::get_table_data ( " sum(fina_cash) cash,sum(fina_credit) credit,count(fina_id) count,fina_action ", TB_PRE . "witkey_finance", " uid=".$arrSellerInfo['uid']." and fina_action in ('pub_task','task_bid','buy_service','sale_service','buy_gy') ", "", " fina_action ", "", "fina_action" );
...
foreach ($arrMarkLists as $k=>$v) {
	if(in_array($v['model_code'],array('goods','service'))){
		if(intval($v['origin_id'])){
			$arrInfo =db_factory::get_one(sprintf('select title from %s where service_id = %d',TB_PRE.'witkey_service',intval($v['origin_id'])));
			$arrMarkLists[$k]['title'] =$arrInfo['title'];
			$arrMarkLists[$k]['model'] ='商品';
			$arrMarkLists[$k]['url'] = 'index.php?do=goods&id='.intval($v['origin_id']);
		}else{
			$arrInfo =db_factory::get_one(sprintf('select title from %s where order_id = %d',TB_PRE.'witkey_service_order',intval($v['obj_id'])));
			$arrMarkLists[$k]['title'] =$arrInfo['title'];
		...
		$arrInfo =db_factory::get_one(sprintf('select task_title from %s where task_id = %d',TB_PRE.'witkey_task',intval($v['origin_id'])));
