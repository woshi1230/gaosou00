...
if (isset ( $action )) {
	switch ($action) {
		case 'mulitDel' :
			if (is_array($ckb)) {
				foreach ( $ckb as $v ) {
					...
					if ($intStatus == 0) {
						db_factory::execute ( "update " . TB_PRE . "witkey_msg set msg_status=1 where msg_id = ".intval($intMsgId) );
					} else {
		...
		case 'delSingle' :
			if ($objId) {
				if ($msgStatus == 0) {
					db_factory::execute ( "update " . TB_PRE . "witkey_msg set msg_status=1 where msg_id = " . intval ( $objId ) );
				} else {
