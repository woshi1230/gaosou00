...
if ($intWithdrawId) {
	$arrWithdrawInfo = db_factory::get_one ( "select * from " . TB_PRE . "witkey_withdraw where withdraw_id=" . intval ( $intWithdrawId ) );
	if ($arrWithdrawInfo ['withdraw_status'] == 2) {
		$strStep = 5;
	}
}
$strStep or $strStep = '1';
$arrPays = kekezu::get_table_data ( "k,v", TB_PRE . "witkey_pay_config", '', '', '', '', 'k' );
$arrPaymentLists = kekezu::get_payment_config (); 
$booVerify = kekezu::reset_secode_session ( $ver ? 0 : 1 ); 
if (isset ( $action ) && $action == 'safe_secode') {
	$strSeccode = keke_user_class::get_password ( $sec_code, $gUserInfo ['passsalt'] );
...
switch ($strStep) {
	...
	case "2" :
		...
		$arrBindLists = kekezu::get_table_data ( "*", TB_PRE . "witkey_auth_bank", "uid='$gUid' and auth_status=1", "", "", "", "bank_id", null );
	...
	case "3" :
		switch ($paymode) {
			case "online" :
				...
				break;
			case "offline" :
				$arrBankInfo = db_factory::get_one ( " select * from " . TB_PRE . "witkey_auth_bank  where uid ='$uid' and auth_status=1 and bank_name='$pay_type'" );
				$arrUserBankInfo = kekezu::get_table_data ( "*", TB_PRE . "witkey_member_bank", 'uid=' . $uid, '', "", '', '' );
				break;
		}
		break;
	case "4" :
