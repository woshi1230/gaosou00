...
switch ($step){
	case "step1":
		if (isset($formhash)&&kekezu::submitcheck($formhash)) {
			...
			$strSql = sprintf(" select card_num from %switkey_member_bank where bind_status=1 and card_num='%s'",TB_PRE,$strCardNum);
		...
		break;
	case "step2":
		$strSql = sprintf(" select * from %switkey_member_bank where bank_id='%d' and uid='%d' and bind_status='1' ",TB_PRE,intval($intBankId),$gUid);
