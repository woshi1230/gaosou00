...
if($orderId){
	...
	$arrServiceOrderInfo = db_factory::get_one(sprintf("select * from %switkey_service_order where order_id=%d",TB_PRE,$orderId));
}
...
if($id == $gUid){
	...
	$arrServiceOrderInfo['indus_pid'] and $arrAllIndustrys = CommonClass::getIndustryByPid($arrServiceOrderInfo['indus_pid'],'catid,parentid,catname');
...
$serviceSql = 'select service_id,model_id,title,price from '.TB_PRE.'witkey_service where uid = '.$id.' and service_status = 2 and model_id = 7 order by service_id desc limit 0,5';
