...
$strTaskSql = 'select * from '.TB_PRE.'witkey_task where task_id = '.$taskId;
...
if($gUserInfo['city']){
	$arrCity = CommonClass::getDistrictById($gUserInfo['city'],'areaid,parentid,areaname');
}
if($gUserInfo['area']){
	$arrArea = CommonClass::getDistrictById($gUserInfo['area'],'areaid,parentid,areaname');
}
...
switch ($op) {
	...
	case "workComment" : 
		if ($strTarComment) {
			...
			$strContent=$arrTaskInfo['username'].'点评了您在<a href='.DT_PATH.'index.php?do=task&id='.$taskId.'>'.$arrTaskInfo[task_title].'</a>中投递的稿件，<a href='.DT_PATH.'index.php?do=task&id='.$taskId.'>快去看看吧</a>';
			MsgClass::SendFeedMsg($wkuid, "您有新的稿件点评", $strContent);
			if($objTask->set_work_comment ( 'work', $intWorkId, $strTarComment )){
				$date = date('Y-m-d',time());
				$time = date('H:i:s',time());
				kekezu::show_msg ( array('d'=>$date,'t'=>$time, 'username'=>$_username), NULL, NULL, NULL, 'ok' );
			}
	...
	case "turnaround" :
		...
		if (isset($formhash)&&kekezu::submitcheck($formhash)){
			...
			require keke_tpl_class::template ( 'task/'.$arrModelInfo['model_code'].'/turnaround' );
		}
		die();
		break;
	case "workvote" : 
	...
	case "report" : 
		...
		if (isset($formhash)&&kekezu::submitcheck($formhash)){
			...
			$strUrl .= '&op=report';
			require keke_tpl_class::template("ajax/report");
		}
		break;
	case "quote" : 
		if(0 === $gUid){
		...
		if (isset($formhash)&&kekezu::submitcheck($formhash)){
         	$arrDistrictData = CommonClass::getAllDistrict('areaid,parentid,areaname');
			...
			require keke_tpl_class::template ( 'task/'.$arrModelInfo['model_code'].'/quote' );
		}
		die();
		break;
	case "matchquote" : 
		$arrHanded = $objTask->work_exists('',"uid='{$uid}'",-1);
		$floatCanPay = $user_info['money'] - $objTask->_task_config['deposit'];
		$arrCoverCash = kekezu::get_cash_cove ( '', true );
		if(0 === $gUid){
			kekezu::show_msg ( '未登陆用户，无权操作', 'index.php?do=login', 3, NULL, 'warning' );
		}
	...
	case 'getContact':
		...
		require keke_tpl_class::template ('task/match/contact');
		die ();
		break;
	case 'sendNotice':
	...
	case 'taskHost':
		if (isset($formhash)&&kekezu::submitcheck($formhash)){
			...
			$twoPassword = keke_user_class::get_password ( $gUserInfo['password'], $gUserInfo['passsalt'] );
			$limit = $objTask->_host_amount;
			require keke_tpl_class::template ('task/match/host');
		}
		die ();
		break;
	case 'workover':
		...
		require keke_tpl_class::template ('task/match/complete');
		die ();
		break;
	case 'taskAccept':
		$objTask->task_accept();
	...
	case "consign" : 
		if (isset($formhash)&&kekezu::submitcheck($formhash)){
			...
			require keke_tpl_class::template ( 'task/'.$arrModelInfo['model_code'].'/consign' );
		}
		die();
	case "plancomplete":
...
//			$delay_cash = keke_curren_class::convert($delay_cash,0,true);
			...
			$floatUserBalance = floatval($gUserInfo['money']);
	...
	case 'workinfo':
		...
		$intFavorite = db_factory::get_count(sprintf('select count(*) from %s where userid = %d and obj_id = %d and keep_type = "work"',TB_PRE.'favorite',intval($gUid),intval($arrWorkInfo['work_id'])));
