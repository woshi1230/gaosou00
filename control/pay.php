...
switch ($type) {
	case 'hosted':
		...
		$arrOrderDetailInfo = db_factory::get_one ( sprintf ( "select order_id from %switkey_order_detail where obj_id=%d and obj_type = 'hosted'", TB_PRE, $id ) );
		$orderId = intval ( $arrOrderDetailInfo ['order_id'] );
		$arrOrderInfo = db_factory::get_one ( sprintf ( "select * from %switkey_order where order_id=%d ", TB_PRE, $orderId ) );
	...
	case 'task' :
		...
		$modelInfo = $kekezu->_model_list [$arrTaskInfo ['model_id']];
		$className = $modelInfo ['model_code'] . "_task_class";
		$arrOrderDetailInfo = db_factory::get_one ( sprintf ( "select order_id from %switkey_order_detail where obj_id=%d and obj_type = 'task'", TB_PRE, $id ) );
		$orderId = intval ( $arrOrderDetailInfo ['order_id'] );
		$arrOrderInfo = db_factory::get_one ( sprintf ( "select * from %switkey_order where order_id=%d ", TB_PRE, $orderId ) );
		$cash = $arrOrderInfo ['order_amount'];
		$title = $arrTaskInfo ['task_title'];
		...
		break;
	case 'order' :
		$arrOrderInfo = db_factory::get_one ( sprintf ( "select order_id,order_amount,order_name,order_uid from %switkey_order where order_id=%d ", TB_PRE, $id ) );
		$arrOrderDetailInfo = db_factory::get_one ( sprintf ( "select obj_id from %switkey_order_detail where order_id=%d ", TB_PRE, $id ) );
		$arrServiceInfo = db_factory::get_one ( sprintf ( "select model_id from %switkey_service where service_id=%d ", TB_PRE, $arrOrderDetailInfo ['obj_id'] ) );
