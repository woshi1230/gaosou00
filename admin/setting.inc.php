...
switch($action) {
	...
	case 'mail':
		...
		if($mail_type == 'sc') {
			...
			$body = '<b>恭喜！您的站点['.$DT['sitename'].']邮件发送设置成功！</b><br/>------------------------------------<br><a href="http://www.gaosou.net/" target="_blank">Send By GAOSOU B2B Mail Tester</a>';
		}	
	...
	case 'static':
		if($itemid) {
			...
			foreach(array(DT_ROOT.'/gaosou.ico', DT_ROOT.'/lang/'.DT_LANG.'/lang.js') as $d) {
				$s = str_replace(DT_ROOT, DT_ROOT.'/file/static', $d);
				file_copy($d, $s);
			}
		}
	...
	break;
	case 'cache':
		$dc->set('gaosou', 'b2b', 3600);
		$pass = $dc->get('gaosou') == 'b2b' ? 1 : 0;
	...
	case 'html':
		tohtml('index');
		$dc->set('gaosou', 'b2b', 3600);
		$dc->get('gaosou') == 'b2b' or dalert('缓存类型'.$CFG['cache'].'测试失败，'.($CFG['cache'] == 'file' ? '请检查file目录是否可写' : '请立即更换'), '?moduleid='.$moduleid.'&file='.$file.'&tab=2');
