...
<tr>
<td class="tl">网站地址</td>
<td><input name="config[url]" type="text" value="<?php echo $url;?>" size="40"/><?php tips('请添写完整URL地址,例如http://www.gaosou.net/<br/>注意以 / 结尾');?></td>
</tr>
	<tr>
		<td class="tl">网站LOGO</td>
		<td><input name="setting[logo]" type="text" value="<?php echo $logo;?>" id="logo" size="58"/> <span onclick="Dthumb(1,180,60, Dd('logo').value, 0, 'logo');" class="jt">[上传]</span>&nbsp;&nbsp;<span onclick="if(Dd('logo').value){Dd('showlogo').src=Dd('logo').value;}" class="jt">[预览]</span>&nbsp;&nbsp;<span onclick="Dd('logo').value='';Dd('showlogo').src='<?php echo DT_SKIN;?>image/logo.png';" class="jt">[删除]</span><br/>
			<a href="<?php echo DT_PATH;?>" target="_blank"><img src="<?php echo $logo ? $logo : DT_SKIN.'image/logo.png';?>" style="margin:2px;" id="showlogo"/></a></td>
	</tr>

	<tr>
		<td class="tl">网站banner</td>
		<td><input name="setting[ad1]" type="text" id="ad1" value="<?php echo $ad1;?>" size="58"/> <span onclick="Dthumb(1,180,60, Dd('ad1').value, 0, 'ad1');" class="jt">[上传]</span>&nbsp;&nbsp;<span onclick="if(Dd('ad1').value){Dd('showad1').src=Dd('ad1').value;}" class="jt">[预览]</span>&nbsp;&nbsp;<span onclick="Dd('ad1').value='';Dd('showad1').src='<?php echo DT_SKIN;?>image/home/ad1.png';" class="jt">[删除]</span><br/>
			<a href="<?php echo DT_PATH;?>" target="_blank"><img src="<?php echo $ad1 ? $ad1 : DT_SKIN.'image/home/ad1.png';?>" style="margin:2px;" id="showad1"/></a></td>
	</tr>
<tr>
<td class="tl">版权信息</td>
...
<td><input name="config[com_domain]" type="text" value="<?php echo $com_domain;?>" size="30"/> <?php tips('如果填写 .gaosou.com 同时需要将域名泛解析 *.gaosou.com 指向服务器IP，并且在服务器端绑定泛域名至 网站根目录 或者 网站根目录/company 目录，生成的主页形式为username.gaosou.com<br/>如果填写 i.gaosou.com 同时需要将域名泛解析 i.gaosou.com 指向服务器IP，并且在服务器端绑定域名至网站根目录/company 目录，生成的主页形式为i.gaosou.com/username/(注：此方式必须支持伪静态)');?></td>
</tr>
<tr>
...
<input type="radio" name="setting[com_www]" value="0"  <?php if(!$com_www){ ?>checked <?php } ?>/> 关闭 <?php tips('例如二级域名为sell.gaosou.com<br/>添加www后为 www.sell.gaosou.com');?>
</td>
</tr>
<tr>
...
<td><input name="setting[remote_url]" type="text" value="<?php echo $remote_url;?>" size="60"/><?php tips('例如 http://static.gaosou.com/，注意以 / 结尾');?></td>
</tr>
<tr> 
...
<tr>
<td class="tl">信任域名</td>
<td><input name="setting[safe_domain]" type="text" value="<?php echo $safe_domain;?>" size="60"/><?php tips('不填写则默认为当前域名<br/>多个域名请用|分开 例如gaosou.com|gaosou.cn');?></td>
</tr>
<tr>
...
<td><input name="config[cookie_domain]" type="text" value="<?php echo $cookie_domain;?>" size="20"/><?php tips('例如要保证顶级域名gaosou.com所有二级域名均可正常登录注销，则填写.gaosou.com(注意顶级域名前加.)');?></td>
</tr>
<tr>
...
<!--<script type="text/javascript" src="https://www.gaosou.net/sms.php?action=get&uid=--><?php //echo DT_CLOUD_UID;?><!--&key=--><?php //echo md5(DT_CLOUD_KEY.'|'.DT_CLOUD_UID);?><!--"></script>-->
