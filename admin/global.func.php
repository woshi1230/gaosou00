...
function tpl($file = 'index', $mod = 'gaosou') {
	global $CFG, $DT;
	return $mod == 'gaosou' ? DT_ROOT.'/admin/template/'.$file.'.tpl.php' : DT_ROOT.'/module/'.$mod.'/admin/template/'.$file.'.tpl.php';
}
...
function tips($tips) {
	echo ' <img src="'.DT_STATIC.'admin/image/help.png" width="11" height="11" title="'.$tips.'" alt="tips" class="c_p" onclick="Dconfirm(this.title, \'\', 450);" />';
}
...
function split_sell($part) {
	...
	$sql = str_replace('gaosou_sell', $db->pre.'sell_'.$part, $sql);
	if($db->version() > '4.1' && $CFG['db_charset']) {
