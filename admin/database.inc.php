...
switch($action) {
	...
	default:
		if(isset($backup)) {
			...
			if(trim($sqldump)) {
				$sqldump = "# gaosou V".DT_VERSION." R".DT_RELEASE." http://www.gaosou.net\n# ".timetodate($DT_TIME, 6)."\n# --------------------------------------------------------\n\n\n".$sqldump;
				$tableid = $i;
			   ...
			   $db->query("DELETE FROM {$DT_PRE}setting WHERE item='gaosou' AND item_key='backtime'");
			   $db->query("INSERT INTO {$DT_PRE}setting (item,item_key,item_value) VALUES('gaosou','backtime','$DT_TIME')");
