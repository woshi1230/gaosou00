...
switch($action) {
	case 'add':
		if($submit) {
			...
			if($post['islink']) {
				...
				if(!file_put(DT_ROOT.'/'.$dir.'/config.inc.php', "gaosou")) msg('目录'.$dir.'无法写入，请设置此目录可写权限');
			}
			...
			if($post['islink']) $post['module'] = 'gaosou';
	...
	case 'remkdir':
		...
		if(is_file($remkdir)) {
			...
			$dir = $r['moduledir'];
			if(!dir_create(DT_ROOT.'/'.$dir)) msg('无法创建'.$dir.'目录，请检查PHP是否有创建权限或手动创建');
			if(!file_put(DT_ROOT.'/'.$dir.'/config.inc.php', "gaosou Test")) msg('目录'.$dir.'无法写入，如果是Linux/Unix服务器，请设置此目录可写权限');
			file_put(DT_ROOT.'/'.$dir.'/config.inc.php', "<?php\n\$moduleid = ".$moduleid.";\n?>");
			include $remkdir;			
			cache_module();
