...
switch($action) {
	case 'download':
		...
		$url = 'http://www.gaosou.net/update.php?product=b2b&release='.$release.'&charset='.DT_CHARSET.'&version='.DT_VERSION;
	...
	case 'unzip':
		...
		if(is_file($release_dir.'/source/gaosou/version.inc.php')) {
	...
	break;
	case 'copy':
		if($CFG['template'] != 'default' && is_dir($release_dir.'/source/gaosou/template/default')) @rename($release_dir.'/source/gaosou/template/default', $release_dir.'/source/gaosou/template/'.$CFG['template']);
		if($CFG['skin'] != 'default' && is_dir($release_dir.'/source/gaosou/skin/default')) @rename($release_dir.'/source/gaosou/skin/default', $release_dir.'/source/gaosou/skin/'.$CFG['skin']);
		$files = file_list($release_dir.'/source/gaosou');
		foreach($files as $v) {
			$file_a = str_replace('file/update/'.$release.'/source/gaosou/', '', $v);
			$file_b = str_replace('source/gaosou/', 'backup/', $v);
		...
		foreach($files as $v) {
			$file_a = str_replace('file/update/'.$release.'/source/gaosou/', '', $v);
			file_copy($v, $file_a) or msg('因文件权限不可写，系统无法覆盖'.str_replace(DT_ROOT.'/', '', $file_a).'<br/>请通过FTP工具移动file/update/'.$release.'/source/gaosou/目录内所有文件覆盖到站点根目录(Windows独立服务器可以直接复制->粘贴)<br/>Linux独立服务器执行\cp -rf '.DT_ROOT.'/file/update/'.$release.'/source/gaosou/* '.DT_ROOT.'/');
		}
