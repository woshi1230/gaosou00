...
if ($op == 'basic') { 
	if ($sbt_recmmend) {
		$res = db_factory::execute ( sprintf ( "update %switkey_task set is_top=1 where task_id='%d' ", TB_PRE, $task_id ) );
...
//			CustomClass::editExtData($task_id, $model_id, $ext_fds);
		...
		$arrFileLists1 = db_factory::query ( sprintf ( " select * from %switkey_file where task_id='%d'  and obj_type='task' ", TB_PRE, $task_id ) );
