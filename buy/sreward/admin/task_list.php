...
switch ($ac) {
	...
	case "stoptask":
		...
		$work_info=db_factory::get_one("select * from  ".TB_PRE."witkey_task_work  where task_id='".intval($task_id)."' and work_status=4");
		if($work_info){
				...
				db_factory::execute("update ".TB_PRE."witkey_task_work set work_status=0 where task_id='".intval($task_id)."' and work_status=4");
		}
	...
	case "unrecommend":
		...
		break;
	case "refund":
		$task_info = db_factory::get_one(" select * from ".TB_PRE."witkey_task where task_id='$task_id'");
