...
$arrTaskInfo = db_factory::get_one(sprintf("select * from %switkey_task where task_id = '%d'",TB_PRE,$arrAgreeInfo['task_id']));
...
// 任务大厅功能，???
$check_right_arr = db_factory::get_one(sprintf("select report_id,username,report_type from %switkey_report where report_status in(1,2) and origin_id ='%d'",TB_PRE,$arrAgreeInfo['task_id']));
...
require keke_tpl_class::template("task/".$arrModelInfo['model_code']."/agreement/index");
die;
