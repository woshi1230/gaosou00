...
class sreward_alipay_trust_class {
	...
	public function init_config() {
		...
		$task_info = db_factory::get_one ( sprintf ( $sql, TB_PRE, TB_PRE, TB_PRE, TB_PRE, $this->_task_id ) );
		$task_info and $task_info ['model_code'] = $kekezu->_model_list [$task_info ['model_id']] ['model_code'];
	...
	function confirm($is_return = false, $data = null) {
		...
		switch ($is_return) {
			case false : 
				...
				$work_info = db_factory::get_one ( sprintf ( $sql, TB_PRE, TB_PRE, $this->_task_id, $data ['work_id'] ) );
			...
			case true : 
				...
				switch ($bidder_info ['transfer_status']) {
					case "W":
						$agree_id = db_factory::get_count(sprintf(" select agree_id from %switkey_agreement where task_id='%d' and work_id='%d'",TB_PRE,$this->_task_id,$bidder_info ['outer_transfer_no']));
	...
	function pt_confirm($is_return = false, $work_id = null) {
		...
		switch ($is_return) {
			...
			case true : 
				switch ($this->_data ['is_success']) {
					case "T" :
						$agree_id = db_factory::get_count ( sprintf ( " select agree_id from %switkey_agreement where task_id='%d'", TB_PRE, $this->_task_id ) );
						$agree_obj = sreward_task_agreement::get_instance ( $agree_id );
			...
			case true : 
				switch($this->_data['is_success']){
					case "T":
						...
						$report_id = db_factory::get_count(sprintf($sql,TB_PRE,TB_PRE,$work_id,$this->_task_id));
