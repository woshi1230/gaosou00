...
//					$wk_get = floatval (keke_curren_class::convert($op_result['wk_get'],0,true));
					...
					if ($total_cash != $gz_get + $wk_get) {
						kekezu::admin_show_msg ( $_lang['wain_you_give_cash_error_notice'], "{$REF['scheme']}://{$REF['host']}{$REF['path']}?{$REF['query']}&report_id=" . $this->_report_id, "3", "", "error" );
					} else {
...
//								$objProm->set_prom_event_status ( $g_event ['parent_uid'], $this->_gusername, $g_event ['event_id'], '3' );
					...
					$res and kekezu::admin_show_msg ( $trans_name . $_lang['deal_success'], "{$REF['scheme']}://{$REF['host']}{$REF['path']}?{$REF['query']}", "3","","success" ) or kekezu::admin_show_msg ( $trans_name . $_lang['deal_fail'], "index.php?do=trans&view=process&type=$type&report_id=" . $this->_report_id, "3","","warning" );
				} else {
					kekezu::admin_show_msg ( $trans_name . $_lang['deal_fail_now_forbit_deal_cash'], "{$REF['scheme']}://{$REF['host']}{$REF['path']}?{$REF['query']}&report_id=" . $this->_report_id,"3","","warning");
				}
				break;
			case "nopass" :
				...
				$res&&$res1 and kekezu::admin_show_msg ( $trans_name . $_lang['deal_success'], "{$REF['scheme']}://{$REF['host']}{$REF['path']}?{$REF['query']}", "3","","success" ) or kekezu::admin_show_msg ( $trans_name . $_lang['deal_fail'], "index.php?do=trans&view=process&type=$type&report_id=" . $this->_report_id, "3","","warning" );
				break;
		}
	}
	public function process_report($op_result, $type) {
		...
		if($op_result ['action']){
			switch ($op_result ['task']) {
				...
				break;
				case 2:
					$arrBids = db_factory::query("select * from ".TB_PRE."witkey_task_work where work_status=4");
					if(is_array($arrBids)){
						foreach($arrBids as $k=>$v){
							db_factory::execute ( sprintf ( "update %switkey_task_work set work_status = 0 where work_id = '%d'", TB_PRE, $v['work_id'] ) );
							db_factory::execute ( sprintf ( " update yw_company set accepted_num = accepted_num-1 where userid = '%d'", $v['uid'] ) );
						}
					}
			...
			if($res){
				kekezu::admin_show_msg ( $trans_name . $_lang['deal_success'], "{$REF['scheme']}://{$REF['host']}{$REF['path']}?{$REF['query']}", "3","","success" );
			}else{
				kekezu::admin_show_msg ( $trans_name . $_lang['deal_fail'], "{$REF['scheme']}://{$REF['host']}{$REF['path']}?{$REF['query']}&report_id=" . $this->_report_id, "3","","warning" );
			}
		}
	}
}