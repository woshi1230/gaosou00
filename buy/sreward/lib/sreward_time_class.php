...
final class sreward_time_class extends time_base_class {
	...
	public function task_hand_end() {
		$task_list = db_factory::query ( sprintf ( " select * from %switkey_task where task_status=2 and  sub_time < '%s' and model_id = '1' and task_union!=2", TB_PRE, time () ) );
	...
	public function task_vote_end() {
		$task_list = db_factory::query ( sprintf ( " select * from %switkey_task where task_status=4 and  sp_end_time < '%s' and model_id = '1' and task_union!=2", TB_PRE, time () ) );
		if (is_array ( $task_list )) {
			foreach ( $task_list as $k => $v ) {
	...
	public function task_choose_end() {
		$task_list = db_factory::query ( sprintf ( " select * from %switkey_task where task_status=3 and  end_time < '%s' and model_id = '1' and task_union!=2 ", TB_PRE, time () ) );
		if (is_array ( $task_list )) {
			foreach ( $task_list as $k => $v ) {
	...
	public function task_notice_end() {
		$task_list = db_factory::query ( sprintf ( " select * from %switkey_task where task_status=5 and  sp_end_time < '%s' and model_id = '1' and task_union!=2", TB_PRE, time () ) );
		if (is_array ( $task_list )) {
			foreach ( $task_list as $k => $v ) {
	...
	public function task_top_end() {
		$task_list = db_factory::query ( sprintf ( " select * from %switkey_task a left join %switkey_payitem_record b
				on a.task_id=b.obj_id where a.tasktop=1 and a.model_id = '1' and  b.obj_type='task' and b.end_time < '%s'  ", TB_PRE,TB_PRE, time () ) );
		if (is_array ( $task_list )) {
			foreach ( $task_list as $k => $v ) {
	...
	public function task_agreement_freeze() {
		...
		$agree_list = db_factory::query ( sprintf ( $sql, TB_PRE, TB_PRE, time () - intval ( $config ['agree_complete_time'] ) * 24 * 3600 ) );
		if (! empty ( $agree_list )) {
			...
			foreach ( $agree_list as $k => $v ) {
				...
				db_factory::execute ( sprintf ( " update %switkey_task set task_status=13 where task_id='%d'", TB_PRE, $v ['task_id'] ) );
				db_factory::execute ( sprintf ( " update %switkey_agreement set agree_status=5 where agree_id='%d'", TB_PRE, $v ['agree_id'] ) );
