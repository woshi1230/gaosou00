...
class sreward_task_class extends keke_task_class {
	...
	public function __construct($task_info) {
		...
		($this->_task_status == '6'||$this->_task_status == '8') and $this->_agree_id = db_factory::get_count ( sprintf ( " select agree_id from %switkey_agreement where task_id='%d'", TB_PRE, $this->_task_id ) );
		$this->init ();
	}
	...
	public function get_work_info($w = array(), $order = null, $p = array()) {
		...
		$work_arr = array ();
		$sql = " select a.*,
						b.seller_credit,b.seller_good_num,b.address,b.seller_total_num,b.seller_level
					from " . TB_PRE . "witkey_task_work a
			   left join yw_company b on a.uid=b.userid";
		...
		if(is_array($work_info)){
			foreach($work_info as $k=>$v){
				...
				if($v['work_file']){
					...
					$work_arr ['work_info'][$k]['attachment'] = db_factory::query('select file_id,save_name,file_name from '.TB_PRE.'witkey_file where file_id in ('.$fileids.')');
				}
				...
				if($this->_task_status==4){
		         ...
		         work_id='%d' and uid='%d' and vote_ip='%s'", TB_PRE, intval($v['work_id']), $this->_uid, kekezu::get_ip () ) );
					if($arrVote){
						$work_arr ['work_info'][$k]['vote'] = true;
					}
					unset($arrVote);
				}
				$arrFavorite = db_factory::get_count(sprintf('select count(*) from %s where userid = %d and obj_id = %d and keep_type = "work"',TB_PRE.'favorite',intval($gUid),intval($v['work_id'])));
				if($arrFavorite){
		...
		$work_ids && $uid == $this->_task_info ['uid'] and db_factory::execute ( 'update ' . TB_PRE . 'witkey_task_work set is_view=1 where work_id in (' . $work_ids . ') and is_view=0' );
		return $work_arr;
	}
	public function work_hand($work_desc, $file_ids, $hidework = '2',$work_hidden='0') {
		...
		if (true === $resText) {
			...
			if ($work_id) {
				$f_ids and db_factory::execute ( sprintf ( " update %switkey_file set work_id='%d',task_title='%s',obj_id='%d' where file_id in (%s)", TB_PRE, $work_id, $this->_task_title, $work_id, $f_ids ) );
				$this->plus_work_num ();
				$this->plus_take_num ();
	...
	public function work_choose($work_id, $to_status, $trust_response = false) {
		...
		if ($to_status == 4) {
			if ($this->_task_status == 7) {
				$cash_info = db_factory::get_one ( sprintf ( " select task_cash,task_union,real_cash from %switkey_task where task_id = '%d'", TB_PRE, $this->_task_id ) );
...
//				}
		...
		$TaskWorkCount=db_factory::execute(sprintf("select * from %switkey_task_work where task_id='%d'",TB_PRE,$this->_task_id));
		$TaskWorkOutCount=db_factory::execute(sprintf("select * from %switkey_task_work where task_id='%d' and work_status=7",TB_PRE,$this->_task_id));
	...
	public function appWorkChoose($work_id, $to_status) {
		global $kekezu, $_K;
		global $_lang;
		$intWorkStatus = db_factory::get_count ( sprintf ( " select work_status from %switkey_task_work where work_id='%d' and uid='%d'", TB_PRE, $work_id, $this->_uid ) );
		if ($intWorkStatus == '8') {
			...
			switch ($to_status) {
				case "4" : 
					$has_bidwork = db_factory::get_count ( sprintf ( " select count(work_id) from %switkey_task_work where work_status='4' and task_id = '%d' ", TB_PRE, $this->_task_id ) );
					if ($has_bidwork) {
						app_class::response ( array (
								'ret' => 107
						) );
...
//				kekezu::save_feed ( $feed_arr, $work_info ['uid'], $work_info ['username'], 'work_accept', $this->_task_id );
	...
	public function set_task_sp_end_time($time_type = 'notice_period') {
		...
		return db_factory::execute ( sprintf ( " update %switkey_task set sp_end_time = '%d' where task_id='%d'", TB_PRE, $sp_end_time, $this->_task_id ) );
	}
	public function set_task_vote($work_id) {
		...
		if ($resText === true) {
			...
			if ($vote_id) {
				db_factory::execute ( sprintf ( " update %switkey_task_work set vote_num=vote_num+1 where work_id ='%d'", TB_PRE, $work_id ) );
				return true;
			} else {
...
//			}
	...
	public function time_hand_end() {
		...
		if ($this->_task_status == 2 && $this->_task_info ['sub_time'] < time ()) {
			$arrOutTask=db_factory::get_one(sprintf("select * from %switkey_task where task_status=2 and  task_id='%d'",TB_PRE,$this->_task_id));
			$TaskWorkCount=db_factory::execute(sprintf("select * from %switkey_task_work where task_id='%d'",TB_PRE,$this->_task_id));
			$TaskWorkOutCount=db_factory::execute(sprintf("select * from %switkey_task_work where task_id='%d' and work_status=7",TB_PRE,$this->_task_id));
			if(($TaskWorkCount==$TaskWorkOutCount) && $arrOutTask){
				$this->dispose_task_return ();
	...
	public function time_vote_end() {
		...
		if ($this->_task_status == 4 && $this->_task_info ['sp_end_time'] < time ()) {
			$bid_work = db_factory::get_one ( sprintf ( " select * from %switkey_task_work where work_status=5 and task_id ='%d' order by vote_num desc,work_time desc limit 0,1", TB_PRE, $this->_task_id ) );
			if ($bid_work ['vote_num'] > 0) {
				...
				db_factory::execute ( sprintf ( " update %switkey_task set is_auto_bid='1' where task_id='%d'", TB_PRE, $this->_task_id ) );
				db_factory::execute ( sprintf ( "update %switkey_task_work set work_status = 0 where work_status=5 and task_id='%d'", TB_PRE, $this->_task_id ) );
...
//				kekezu::save_feed ( $feed_arr, $bid_work ['uid'], $bid_work ['username'], 'work_accept', $this->_task_id );
				...
				db_factory::execute ( sprintf ( " update %switkey_task_work set work_status='7' where work_status = '5' and task_id = '%d'", TB_PRE, $this->_task_id ) );
				$this->dispose_task_return ();
			}
		}
	}
...
//					}
	...
	public function create_agree_date($work_info) {
		...
		$agree_id = yw_task_agreement::create_agreement ( $agree_title, $this->_model_id, $this->_task_id, $work_info ['work_id'], $this->_guid, $work_info ['uid'] );
...
//									left join yw_company c on a.uid=c.userid
//								where a.task_id='%d' and a.work_status='0' order by %s,a.work_time asc limit 0,%d";
							$sql = "select a.* from %switkey_task_work a
									left join yw_company c on a.uid=c.userid
								where a.task_id='%d' and a.work_status='0' order by %s,a.work_time asc limit 0,%d";
							$work_list = db_factory::query ( sprintf ( $sql, TB_PRE, $this->_task_id, $order_field, $split_num ) );
...
//							$prom_obj->dispose_prom_event ( "pub_task", $this->_guid, $this->_task_id );
	...
	public function check_if_voted($work_id) {
		...
		$vote_count = db_factory::get_count ( sprintf ( " select count(vote_id) from %switkey_vote where
		 work_id='%d' and uid='%d' and vote_ip='%s'", TB_PRE, $work_id, $this->_uid, kekezu::get_ip () ) );
		if ($vote_count > 0) {
	...
	public function dispose_order($order_id, $trust_response = false) {
		...
		$task_status = $this->_task_status;
		$order_info = db_factory::get_one ( "select * from " . TB_PRE . "witkey_order where order_id=" . intval ( $order_id ) );
...
//					}
					...
					db_factory::execute ( sprintf ( " update %switkey_task set cash_cost='%s',credit_cost='%s' where task_id='%d'", TB_PRE, $consume ['cash'], $consume ['credit'], $this->_task_id ) );
...
//						kekezu::save_feed ( $feed_arr, $task_info ['uid'], $task_info ['username'], 'pub_task', $task_info ['task_id'] );
	...
	public static function master_opera($m_id, $t_id, $url, $t_cash) {
		...
		switch ($status) {
			...
			case 5 :
				...
				break;
			case 6 :
				$agree_id = db_factory::get_count ( sprintf ( ' select agree_id from %switkey_agreement where task_id=%d and buyer_uid=%d', TB_PRE, $t_id, $uid ) );
				$button ['agree'] = array (
	...
	public static function wiki_opera($m_id, $t_id, $w_id, $url) {
		...
		$model_id = db_factory::get_count ( sprintf ( ' select model_id from %switkey_task where task_id=%d', TB_PRE, $t_id, $uid ), 0, 'model_id', 600 );
		$intWorkStatus = db_factory::get_count ( sprintf ( ' select work_status from %switkey_task_work where work_id=%d', TB_PRE, $w_id, $uid ), 0, 'work_status', 600 );
		$site = $_K ['siteurl'] . '/';
		$button = array ();
		$button ['view'] = array (
		...
		switch ($status) {
			...
			case 13 :
				  if ($intWorkStatus == 4 && $model_id == 1) {
					$agree_id = db_factory::get_count ( sprintf ( ' select agree_id from %switkey_agreement where task_id=%d and seller_uid=%d', TB_PRE, $t_id, $uid ) );
					$button ['agree'] = array (
							'href' => $site . 'index.php?do=agreement&taskId='.$t_id.'&agreeId=' . $agree_id,
							'desc' => '交付详情',
					);
	...
	public static function getUsername($uid){
		$strSql="select * from yw_company where userid=".intval($uid);
		$arrResult=db_factory::get_one($strSql);
		return $arrResult['username'];
	}
	public static function getTitle($task_id){
		$strSql="select * from ".TB_PRE."witkey_task where task_id=".intval($task_id);
		$arrResult=db_factory::get_one($strSql);
		return $arrResult['task_title'];
	}
	public static function getSeverTitle($service_id){
		$strSql="select * from ".TB_PRE."witkey_service where service_id=".intval($service_id);
		$arrResult=db_factory::get_one($strSql);
