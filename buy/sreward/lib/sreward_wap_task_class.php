...
class sreward_wap_task_class extends sreward_task_class {
	...
	public function wap_work_count(){
		$data = array();
		$c = kekezu::get_table_data('count(work_id) c,work_status',TB_PRE . 'witkey_task_work',' task_id='.$this->_task_id,'',' work_status','','work_status');
	...
	public function get_task_file(){
		...
		if($f){
			$data=db_factory::query(" select file_name as name,CONCAT('".$_K['siteurl']."/',`save_name`) path from ".TB_PRE."witkey_file where file_id in (".$f.")");
		}
		return $data;
	}
	...
	public function wap_favor(){
		kekezu::set_favor('task_id', 'task', $this->_model_id, $this->_task_info[uid], $this->_task_id, $this->_task_title, $this->_task_id,'','json');
	}
	...
	public function wap_list() {
		...
		$info = db_factory::query(sprintf('select * from %switkey_task_work where task_id=%d %s limit %d,%d',TB_PRE,$this->_task_id,$w,$ls,$le),1,3600);
		$count = db_factory::get_count('select count(work_id) from '.TB_PRE.'witkey_task_work where task_id='.$this->_task_id.$w);
		if($info){
			$ids   = implode(',',array_keys($info));
			$f_tmp = kekezu::get_table_data("obj_id,CONCAT('".$_K['siteurl']."/',`save_name`) file",TB_PRE . 'witkey_file'," obj_id in ('".ids."') and obj_type='work'",'','','','obj_id',3600);
	...
	public function wap_workinfo($work_id){ 
		global $_K;
		$work_info =  db_factory::get_one("select * from ".TB_PRE."witkey_task_work where work_id=".intval($work_id));
		if($work_info[work_file]){
			$file_arr = db_factory::query("select file_id,file_name,save_name from ".TB_PRE."witkey_file where file_id in ($work_info[work_file])");
	...
	public function wap_upload($f='uploadedfile',$work_id=0){
		...
		if($path){
			$fid = db_factory::get_count(sprintf(" select file_id from %switkey_file where save_name='%s'",TB_PRE,$path));
			if($work_id){
				db_factory::execute(sprintf("update %switkey_task_work set work_file='%d' where task_id='%d' and work_id='%d'",TB_PRE,$fid,$this->_task_id,$work_id));
				db_factory::execute(sprintf(" update %switkey_file set obj_type='work',task_id='%d',obj_id='%d' where file_id='%d'",TB_PRE,$this->_task_id,$work_id,$fid));
			}else{
				db_factory::execute(sprintf("update %switkey_task set task_file='%d' where task_id='%d'",TB_PRE,$fid,$this->_task_id));
				db_factory::execute(sprintf(" update %switkey_file set obj_type='task',task_id='%d' where file_id='%d'",TB_PRE,$this->_task_id,$fid));
			}
			kekezu::echojson('',1);
		}else{
