		...
		if ($ac && $bid_id) {
			switch ($ac) {
				case 'del' : 
					...
					$res = db_factory::execute ( sprintf ( 'delete  from %switkey_task_bid where bid_id=%d', TB_PRE, intval ( $bid_id ) ) );
					if ($res) {
						db_factory::execute ( sprintf ( ' delete from %switkey_comment where obj_id=%d', TB_PRE, intval ( $bid_id ) ) );
					}
				...
				case 'comm' : 
					$c_list = db_factory::query ( sprintf ( ' select a.content,a.on_time from %switkey_comment a 
						left join %switkey_task_work b on a.obj_id=b.work_id where b.work_id=%d', TB_PRE, TB_PRE, $bid_id ) );
					break;
			}
