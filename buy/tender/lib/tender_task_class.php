...
class tender_task_class extends keke_task_class {
	...
	public function tender_work_hand($work_info) {
		...
		if ($this->check_if_can_hand () === true) {
			...
			$this->notify_user ( "task_hand", $_lang ['task_hand'], $v_arr, 2, $this->_guid );
...
//				kekezu::save_feed ( $feed_arr, $bid_info ['uid'], $bid_info ['username'], 'work_accept', $this->_task_info ['task_id'] );
	...
	public function get_work_info($w = array(), $order = null, $p = array()) {
		...
		$work_arr = array ();
		$sql = " select a.* from " . TB_PRE . "witkey_task_work a left join yw_company b on a.uid=b.userid";
		...
		$work_ids&&$uid==$this->_task_info['uid'] and db_factory::execute('update '.TB_PRE.'witkey_task_bid set is_view=1 where bid_id in ('.$work_ids.') and is_view=0');
		return $work_arr;
	}
	...
	public static function wiki_opera($m_id,$t_id, $w_id,$url) {
		...
		switch ($status) {
			case 2 :
				break;
			case 4 :
				$is_op=db_factory::get_one("select * from ".TB_PRE."witkey_task_bid where uid='".intval($uid)."' and task_id='".$t_id."' and bid_status=4");
				if($is_op){
