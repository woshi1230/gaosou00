...
final class tender_time_class extends time_base_class {
	...
	function task_tbtime_out() {
		global $_lang;
		$sql = sprintf ( "select * from %switkey_task where model_id = 4 and task_status=2 and " . time () . ">sub_time", TB_PRE );
	...
	function task_xbtime_out() {
		global $_lang;
		$sql = sprintf ( "select * from %switkey_task where model_id = 4 and task_status=3 and " . time () . ">end_time", TB_PRE );
		$task_arr = db_factory::query ( $sql );
		foreach ( $task_arr as $k => $v ) {
	...
	public function task_top_end() {
		$task_list = db_factory::query ( sprintf ( " select * from %switkey_task a left join %switkey_payitem_record b
				on a.task_id=b.obj_id where a.tasktop=1 and a.model_id = '4' and  b.obj_type='task' and b.end_time < '%s'  ", TB_PRE,TB_PRE, time () ) );
