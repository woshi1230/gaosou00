...
if($report_info ['obj']=='task'||$report_info ['obj']=='work'){
	$task_cash = db_factory::get_one(sprintf("select single_cash,profit_rate from %switkey_task where task_id='%d'",TB_PRE,$report_info ['origin_id'] ));
	$plan_cash_arr = db_factory::query(sprintf("select * from %switkey_task_plan where task_id = '%d'",TB_PRE,$report_info ['origin_id'] ));
	$plan_cash = '';
