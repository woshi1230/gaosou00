	...
	public function bid_init() {
		if ($this->_task_status >= 4) {
			$this->_bid_info = db_factory::get_one ( sprintf ( " select *  from %switkey_task_bid where
			 bid_status = '4' and task_id = '%d'", TB_PRE, $this->_task_id ) );
		} else {
			$this->_bid_info = null;
		}
	}
...
//							)
//					);
//					$arrTaskInfo = db_factory::get_one ( "select * from " . TB_PRE . "witkey_task where task_id=" . intval ( $this->_task_id ) );
//					kekezu::save_feed ( $feed_arr, $arrTaskInfo ['uid'], $arrTaskInfo ['username'], 'task_baoming_bei', $this->_task_id );
	...
	public function check_bid() {
		$count = db_factory::get_count ( sprintf ( "select count(*) from %switkey_task_bid where task_id='%d' and uid='%d'", TB_PRE, $this->_task_id, $this->_uid ) );
		if (intval ( $count ) == 0) {
			return true;
		} else {
	...
	public function bid_edit($bid_id, $quote, $cycle, $area, $work_desc, $plan_amount, $start_time, $end_time, $plan_title) {
		...
		if ($this->_process_can ['work_edit']) {
			...
			db_factory::execute ( sprintf ( "delete from %switkey_task_plan where bid_id='%d'", TB_PRE, $bid_id ) );
			$size = sizeof ( $plan_amount );
			for($i = 0; $i < $size; $i ++) {
				$plan_info = array (
						'plan_amount' => $plan_amount [$i],
	...
	public function work_choose($bid_id, $to_status, $trust_response = false) {
		...
		if ($this->set_bid_status ( $bid_id, $to_status )) {
			if ($this->_task_status == 3) {
				$arrOutTask = db_factory::get_one ( sprintf ( "select * from %switkey_task where task_status=3 and  task_id='%d'", TB_PRE, $this->_task_id ) );
				$TaskWorkCount = db_factory::execute ( sprintf ( "select * from %switkey_task_bid where task_id='%d'", TB_PRE, $this->_task_id ) );
				$TaskWorkOutCount = db_factory::execute ( sprintf ( "select * from %switkey_task_bid where task_id='%d' and bid_status=7", TB_PRE, $this->_task_id ) );
...
//				$objProm->create_prom_event('pub_task', $this->_task_info ['uid'], $this->_task_id);
	...
	public function check_if_operated($work_id, $to_status, $url = '', $output = 'normal') {
		...
		if ($this->check_if_can_choose ( $url, $output )) {
			$work_status = db_factory::get_count ( sprintf ( " select bid_status from %switkey_task_bid where bid_id='%d' and uid='%d'", TB_PRE, $work_id, $this->_uid ) );
			if ($work_status == '8') {
				...
				switch ($to_status) {
					case "4" :
						$has_bidwork = db_factory::get_count ( sprintf ( " select count(bid_id) from %switkey_task_bid where bid_status='4' and task_id='%d' ", TB_PRE, $this->_task_id ) );
						if ($has_bidwork) {
	...
	public function hosted_amount() {
		...
		if ($this->_bid_info) {
			...
			$exist_id = db_factory::get_count ( sprintf ( "select b.order_id from %switkey_order_detail a left join %switkey_order b on a.order_id=b.order_id left join %switkey_task c on a.obj_id=c.task_id where b.order_status='wait' and a.obj_type='hosted' and c.task_id='%d'", TB_PRE, TB_PRE, TB_PRE, $this->_task_id ) );
			if ($amount >= 0) {
				if ($amount > 0) {
					$data = array (
				...
				if ($exist_id) {
					$order_status == 'ok' and db_factory::execute ( sprintf ( "update %switkey_order set order_status='ok' where order_id='%d'", TB_PRE, $exist_id ) );
				}
			}
			if ($res || $amount == 0) {
				$this->set_task_status ( 6 );
				db_factory::execute ( sprintf ( "update %switkey_task set single_cash='%d',credit_cost = credit_cost+'%d',cash_cost = cash_cost+'%d' where task_id='%d'", TB_PRE, floatval ( $this->_bid_info ['quote'] ), $credit_cost, $cash_cost, $this->_task_id ) );
				$v_arr = array (
	...
	public function plan_complete($plan_id, $plan_step) {
		...
		if (is_array ( $plan_ids ) && in_array ( $plan_id, $plan_ids )) {
			if ($this->set_plan_status ( 1, $plan_id )) {
				db_factory::execute ( sprintf ( "update %switkey_task_plan set over_time = %d where plan_id=%d and plan_step=%d", TB_PRE, time () + $config ['confirm_limit_time'] * 3600, $plan_id, $plan_step ) );
				$v_arr = array (
						$_lang ['username'] => $this->_gusername,
						$_lang ['model_name'] => $this->_model_name,
						$_lang ['task_id'] => $this->_task_id,
...
//				$objProm->dispose_prom_event('bid_task', $uid, $obj_id);
	...
	public function task_confirm_timeout() {
		...
		if ($this->_task_status == 6) {
			...
			$set_plan = db_factory::get_one ( sprintf ( "select * from %switkey_task_plan where task_id=%d and plan_id in ($ids_str) and plan_status=1 and over_time<%d", TB_PRE, $this->_task_id, time () ) );
...
//					$objProm->dispose_prom_event ( "bid_task", $this->_bid_info ['uid'], $this->_task_id );
	...
	public function getTuoGuanCash($task_id){
	    $sum=db_factory::get_one("select sum(quote) as sum from ".TB_PRE."witkey_task_bid where task_id =".intval($task_id));
	   return $sum['sum'];
	}
...
//					$objProm->dispose_prom_event ( "bid_task", $this->_bid_info ['uid'], $this->_task_id );
	...
	public function plan_cash($plan_id, $title_url) {
		global $_K, $kekezu;
		global $_lang;
		$plan_info = db_factory::get_one ( sprintf ( "select * from %switkey_task_plan where plan_id='%d'", TB_PRE, $plan_id ) );
...
//			kekezu::save_feed ( $feed_arr, $bid_info ['uid'], $bid_info ['username'], 'work_accept', $this->_task_info ['task_id'] );
	...
	public function get_plan_ids() {
		$this->_bid_info and $plan_ids = db_factory::query ( sprintf ( "select plan_id from %switkey_task_plan where bid_id='%d'", TB_PRE, $this->_bid_info ['bid_id'] ) );
		if ($plan_ids) {
	...
	public function set_plan_status($to_status, $plan_id) {
		return db_factory::execute ( sprintf ( "update %switkey_task_plan set plan_status='%d' where plan_id='%d'", TB_PRE, $to_status, $plan_id ) );
	}
...
//		}
	...
	public function get_work_info($w = array(), $order = null, $p = array()) {
		...
		$work_arr = array ();
		$sql = " select * from " . TB_PRE . "witkey_task_bid a left join yw_company b on a.uid=b.userid";
		$count_sql = " select count(a.bid_id) from " . TB_PRE . "witkey_task_bid a left join yw_company b on a.uid=b.userid";
		...
		$plan_sql = " select * from " . TB_PRE . "witkey_task_plan where task_id = '$this->_task_id' and bid_id in($bid_id_str) order by plan_step asc  ";
		...
		$bid_id_str && $uid == $this->_task_info ['uid'] and db_factory::execute ( 'update ' . TB_PRE . 'witkey_task_bid set is_view=1 where bid_id in (' . $bid_id_str . ') and is_view=0' );
		return $arrWorks;
	}
	public function get_single_bid($bid_id) {
		$bid_info = db_factory::get_one ( sprintf ( "select * from %switkey_task_bid where bid_id='%d'", TB_PRE, $bid_id ) );
		if ($bid_info) {
			$plan_info = db_factory::query ( sprintf ( "select * from %switkey_task_plan where bid_id='%d'order by start_time", TB_PRE, $bid_id ) );
	...
	public function set_bid_status($bid_id, $to_status) {
		return db_factory::execute ( sprintf ( " update %switkey_task_bid set bid_status='%d' where bid_id='%d'", TB_PRE, $to_status, $bid_id ) );
	}
	public function set_task_sp_end_time() {
		$sp_end_time = time () + $this->_task_config ['pay_limit_time'] * 24 * 3600;
		return db_factory::execute ( sprintf ( " update %switkey_task set sp_end_time = '%d'", TB_PRE, $sp_end_time ) );
	}
	...
	public function getWorkInfo($w = array(), $p = array()) {
		global $kekezu, $uid;
		$arrWorks = array ();
		$strSql = " select a.*,b.seller_credit,b.seller_good_num,b.seller_total_num,b.seller_level from " . TB_PRE . "witkey_task_bid a left join yw_company b on a.uid=b.userid";
		...
		$strWorkIds && $uid == $this->_task_info ['uid'] and db_factory::execute ( 'update ' . TB_PRE . 'witkey_task_bid set is_view=1 where bid_id in (' . $strWorkIds . ') and is_view=0' );
		return $arrWorks;
	}
	function getPlan($id) {
		return db_factory::query ( sprintf ( 'select * from %s where task_id = %d and bid_id = %d order  by plan_step asc ', TB_PRE . 'witkey_task_plan', $this->_task_id, $id ) );
	}
	public function dispose_order($order_id) {
		...
		$order_info = db_factory::get_one ( sprintf ( "select * from %switkey_order where order_id='%d'", TB_PRE, $order_id ) );
		...
		if ($order_info ['order_status'] == 'ok') {
			...
			if ($leave_cash >= 0) {
				$order_type = db_factory::get_count ( sprintf ( "select obj_type from %switkey_order_detail where order_id='%d' and obj_type in('hosted','task') ", TB_PRE, $order_id ) );
...
//							kekezu::save_feed ( $feed_arr, $task_info ['uid'], $task_info ['username'], 'pub_task', $task_info ['task_id'] );
						...
						db_factory::execute ( sprintf ( " update %switkey_task set cash_cost='%s' where task_id='%d'", TB_PRE, $task_info ['task_cash'], $this->_task_id ) );
					}
