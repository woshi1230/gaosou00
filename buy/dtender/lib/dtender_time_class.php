...
final class dtender_time_class extends time_base_class {
	...
	public function task_hand_end() {
		$task_list = db_factory::query(sprintf("select * from %switkey_task where task_status=2 and sub_time<'%s' and model_id=5",TB_PRE,time()));
	...
	public  function task_tg_end(){
		$task_list = db_factory::query(sprintf("select * from %switkey_task where task_status=4 and sp_end_time<'%s' and model_id=5",TB_PRE,time()));
		if(is_array($task_list)){
			foreach($task_list as $v){
				$task_obj = new dtender_task_class($v);
	...
	function task_choose_end() {
		$task_list = db_factory::query(sprintf("select * from %switkey_task where task_status=3 and end_time<'%s' and model_id=5",TB_PRE,time()));
		if(is_array($task_list)){
			foreach($task_list as $v){
				$task_obj = new dtender_task_class($v);
	...
	function plan_confirm_end(){
		...
		$task_list = db_factory::query(sprintf($sql,TB_PRE,TB_PRE,time()));
		if(is_array($task_list)){
			foreach($task_list as $v){
				$task_obj = new dtender_task_class($v);
	...
	function task_finish_auto_mark(){
		$nomark_wk_list = db_factory::query(sprintf('select `mark_id` from %switkey_mark where model_code="%s" and mark_status=0 and mark_max_time<%d and mark_type=1',TB_PRE,'dtender',time()));
		...
		$nomark_gz_list = db_factory::query(sprintf('select `mark_id` from %switkey_mark where model_code="%s" and mark_status=0 and mark_max_time<%d and mark_type=2',TB_PRE,'dtender',time()));
	...
	public function task_top_end() {
		$task_list = db_factory::query ( sprintf ( " select * from %switkey_task a left join %switkey_payitem_record b
				on a.task_id=b.obj_id where a.tasktop=1 and a.model_id = '5' and  b.obj_type='task' and b.end_time < '%s'  ", TB_PRE,TB_PRE, time () ) );
		if (is_array ( $task_list )) {
