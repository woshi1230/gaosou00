...
if($task_id){
	...
	switch ($ac) {
		case "del" : 
			$task_title = db_factory::get_count(sprintf("select task_title from %switkey_task where task_id='%d' ",TB_PRE,$task_id));
			...
			case "settask":
				...
				break;
			case "stoptask":
				$model_info 		= db_factory::get_one("select * from ".TB_PRE."witkey_model where model_id=3");
				$config				= unserialize($model_info[config]);
				$task_info 			= get_task_info($task_id);
				$bid_sum			= db_factory::get_count("select count(*) as sum from ".TB_PRE."witkey_task_work where task_id='".intval($task_id)."' and work_status=6");
				...
				$arrWorkInfo=db_factory::query("select * from ".TB_PRE."witkey_task_work where task_id='".intval($task_id)."' and work_status=6 group by uid");
