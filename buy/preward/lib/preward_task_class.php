...
class preward_task_class extends keke_task_class {
	...
	public function get_work_info($w = array(), $p = array()) {
		global $kekezu, $uid;
		$arrWorks = array ();
		$strSql = " select a.*,b.seller_credit,b.seller_good_num,b.seller_total_num,b.seller_level from " . TB_PRE . "witkey_task_work a left join yw_company b on a.uid=b.userid";
	...
	public function get_work_count($where) {
		if ($where == 'max') {
			...
			$count = db_factory::get_count ( sprintf ( "select count(work_id) from %switkey_task_work where %s and task_id='%d'", TB_PRE, $where, $this->_task_id ) );
		}
		return intval ( $count );
	}
...
//							$objProm->dispose_prom_event ( "pub_task", $this->_guid, $this->_task_id );
							if ($this->_task_info ['task_union'] > 1) {
								$bid_uid = array ();
								$ids = db_factory::query ( 'select uid from ' . TB_PRE . 'witkey_task_work where work_status=6 and task_id=' . $this->_task_id );
								foreach ( $ids as $v ) {
									$bid_uid [] = $v ['uid'];
								}
							}
...
//				$objProm = keke_prom_class::get_instance ();
				...
				if (intval ( $hege_count ) < intval ( $this->_task_info ['work_count'] )) {
					if (intval ( $this->_task_config ['is_auto_adjourn'] ) == 1) {
						...
						$work_list = db_factory::query ( sprintf ( "select * from %switkey_task_work where task_id='%d' and work_status=0 order by work_time asc limit 0,%d", TB_PRE, $this->_task_id, intval ( $auto_num ) - intval ( $hege_count ) ) );
						if ($work_list) {
