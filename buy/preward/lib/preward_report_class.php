			...
			switch ($op_result ['task']) {
				...
				case 2:
					if (intval ( $this->_task_obj->_task_config ['is_auto_adjourn'] ) == 1) {
						...
						$auto_num and $work_list = db_factory::query ( sprintf ( "select * from %switkey_task_work where task_id='%d' and work_status=0 order by work_time asc limit 0,%d", TB_PRE, $this->_task_obj->_task_id, $auto_num ) );
						if ($work_list) {
				...
				case 5:
					...
					$intHegeCount = $this->_task_obj->get_work_count ( "work_status=6" );
					$auto_num = intval ( $this->_task_info ['work_count'] )-$intHegeCount;
					$work_list = db_factory::query ( sprintf ( "select * from %switkey_task_work where task_id='%d' and work_status=0 order by work_time asc limit 0,%d", TB_PRE, $this->_task_obj->_task_id, $auto_num ) );
						if ($work_list) {
							foreach ( $work_list as $v ) {
								$this->_task_obj->set_work_status ( $v ['work_id'], 6 );
								$task_id = $this->_task_obj->_task_id;
