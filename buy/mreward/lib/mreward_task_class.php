...
class mreward_task_class extends keke_task_class {
	...
	public function task_requirement_init() {
		...
		$require_arr [$_lang ['haved_bid_work']] = $bid_num = intval ( db_factory::get_count ( sprintf ( " select count(work_id) count from %switkey_task_work where
		 work_status = '4' and task_id = '%d'", TB_PRE, $this->_task_id ) ) );
	}
...
public function getProjectProgressDesc() {
	...
	public function get_work_info($w = array(),  $p = array()) {
		...
		$work_arr = array ();
		$sql = " select a.*,b.seller_credit,b.seller_good_num,b.seller_total_num,b.seller_level from " . TB_PRE . "witkey_task_work a left join yw_company b on a.uid=b.userid";
		...
		if(is_array($work_info)){
			foreach($work_info as $k=>$v){
				...
				if($v['work_file']){
					...
					$arrWorks ['work_info'][$k]['attachment'] = db_factory::query('select file_id,save_name,file_name from '.TB_PRE.'witkey_file where file_id in ('.$fileids.')');
				}
...
//				}
	...
	public function appWorkChoose($work_id, $to_status) {
		global $_K, $kekezu;
		global $_lang;
		$work_status = db_factory::get_count ( sprintf ( " select work_status from %switkey_task_work where work_id='%d'", TB_PRE, $work_id ) );
		if ($work_status == '8') {
...
//			}
	...
	public static function task_delay($task_id, $task_cash, $delay_cash) {
		$prize_data = db_factory::query ( sprintf ( " select * from %switkey_task_prize where task_id='%d'", TB_PRE, $task_id ) );
		foreach ( $prize_data as $v ) {
			$rate = $v ['prize_cash'] / $task_cash;
			$new_cash = $v ['prize_cash'] + $delay_cash * $rate;
			db_factory::execute ( sprintf ( " update %switkey_task_prize set prize_cash='%.2f' where prize_id='%d'", TB_PRE, $new_cash, $v ['prize_id'] ) );
		}
	}
...
//			$objProm->set_prom_event_status ( $p_event ['parent_uid'], $this->_gusername, intval ( $p_event ['event_id'] ), '3' );
	...
	public function auto_task_return() {
		global $kekezu;
			$arrPrizeWork = db_factory::query ( sprintf ( "select * from %switkey_task_work where task_id='%d' and work_status in(1,2,3) ", TB_PRE, $this->_task_id ) );
	...
	public function task_xg_timeout() {
		...
		if (time () > $this->_task_info ['end_time'] && $this->_task_info ['task_status'] == 3) {
			...
			$work_num = $this->_task_info ['work_num'];
			$bid_count = db_factory::get_count ( sprintf ( "select count(work_id) as work_count from %switkey_task_work where task_id='%d' and work_status in (1,2,3) ", TB_PRE, $this->_task_id ) );
			if (! $bid_count) {
...
//		$prom_obj =$objProm = keke_prom_class::get_instance ();
		if (time () > $this->_task_info ['sp_end_time'] && $this->_task_info ['task_status'] == 5) {
			$prize_work_arr = db_factory::query ( sprintf ( "select * from %switkey_task_work where task_id='%d' and work_status in(1,2,3) ", TB_PRE, $this->_task_id ) );
			$prize_date = $this->get_prize_date ();
			$bid_uid = array ();
...
//			$prom_obj->dispose_prom_event ( "pub_task", $this->_guid, $this->_task_id );
	...
	public function auto_choose($prize1_num, $prize2_num, $prize_all) {
		...
		global $_lang;
//		$prom_obj = $objProm = keke_prom_class::get_instance ();
		$has_bided = db_factory::get_count ( sprintf ( " select count(work_id) from %switkey_task_work where work_status in (1,2,3) and task_id ='%d'", TB_PRE, $this->_task_id ) );
		if (! $has_bided) {
			switch ($this->_task_config ['end_action']) {
				case "split" :
					...
					if (in_array ( $split_style, array (
						...
						$work_bid_arr = db_factory::query ( sprintf ( $sql, TB_PRE, $this->_task_id, $order_field, $prize_all ) );
...
//							}
	...
	public function reportAutoChoose(){
		...
		global $_lang;
		$arrPrizeDate = $this->get_prize_date ();
		$arrBidWork = kekezu::get_table_data('count(work_id) as num,work_status',TB_PRE . 'witkey_task_work','work_status in (1,2,3) and task_id ='.$this->_task_id,'','work_status','','work_status','');
				...
				case "split" :
					...
					if (in_array ( $split_style, array (
						...
						$sql = "select a.* from %switkey_task_work a
									left join yw_company c on a.uid=c.userid
								where a.task_id='%d' and a.work_status='0' order by %s,a.work_time asc limit 0,%d";
						$work_bid_arr = db_factory::query ( sprintf ( $sql, TB_PRE, $this->_task_id, $order_field, $intPrizeAll ) );
						if($work_bid_arr){
							foreach ( $work_bid_arr as $k => $v ) {
	...
	public function get_task_prize() {
		$task_prize_arr = kekezu::get_table_data ( "*", TB_PRE . "witkey_task_prize", "task_id={$this->_task_id}", "", "", "", "prize", 0 );
	...
	public function get_prize_date() {
		...
		$count = array ();
		$cash = array ();
		$prize_arr = db_factory::query ( sprintf ( "select * from %switkey_task_prize where task_id='%d' ", TB_PRE, $this->_task_id ) );
		$i = 1;
	...
	public function get_prize_work_count() {
		...
		$work_count_arr = db_factory::query ( sprintf ( "select work_status,count(work_id)  as work_count from %switkey_task_work where task_id='%d' and work_status in(1,2,3) GROUP BY work_status ", TB_PRE, $this->_task_id ) );
