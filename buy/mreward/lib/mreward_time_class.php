...
final class mreward_time_class extends time_base_class {
	...
	function  task_tg_timeout(){
		$task_tg = db_factory::query(sprintf("select * from %switkey_task where model_id=2 and task_status=2 and '%d' > sub_time",TB_PRE,time()));
		if(is_array($task_tg)){
	...
	function task_xg_timeout() {
		$task_xg = db_factory::query(sprintf("select * from %switkey_task where model_id=2 and task_status=3 and '%d' > end_time",TB_PRE,time()));
		if(is_array($task_xg)){
	...
	function task_gs_timeout() {
		$task_gs = db_factory::query(sprintf("select * from %switkey_task where model_id=2 and task_status=5 and '%d' > sp_end_time",TB_PRE,time()));
		if(is_array($task_gs)){
	...
	public function task_top_end() {
		$task_list = db_factory::query ( sprintf ( " select * from %switkey_task a left join %switkey_payitem_record b
				on a.task_id=b.obj_id where a.tasktop=1 and a.model_id = '2' and  b.obj_type='task' and b.end_time < '%s'  ", TB_PRE,TB_PRE, time () ) );
