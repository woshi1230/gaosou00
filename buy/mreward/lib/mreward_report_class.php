		...
		if($op_result ['action']){
			switch ($op_result ['task']) {
				...
				case 2:
					...
					$bid_count = db_factory::get_count ( sprintf ( "select count(work_id) as work_count from %switkey_task_work where task_id='%d' and work_status in (1,2,3) ", TB_PRE, $this->_task_obj->_task_id) );
