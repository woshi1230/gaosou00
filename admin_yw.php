...
if($CFG['authadmin'] == 'cookie') {
	$_gaosou_admin = get_cookie($secretkey);
	$_gaosou_admin = $_gaosou_admin ? intval($_gaosou_admin) : 0;
} else {
	$session = new dsession();
	$_gaosou_admin = isset($_SESSION[$secretkey]) ? intval($_SESSION[$secretkey]) : 0;
}
$_founder = $CFG['founderid'] == $_userid ? $_userid : 0;
$_catids = $_childs = '';
$_catid = $_child = array();
if($file != 'login') {
	if($_groupid != 1 || $_admin < 1 || !$_gaosou_admin) msg('', '?file=login&forward='.urlencode($DT_URL));
	if(!admin_check()) {
		admin_log(1);
