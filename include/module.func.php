...
function get_albums($item, $type = 0) {
	$imgs = array();
	if($item['thumb'] && !preg_match("/^[a-z0-9_\-\.\:\/]{30,}$/i", $item['thumb'])) $item['thumb'] = '';
	if($item['thumb1'] && !preg_match("/^[a-z0-9_\-\.\:\/]{30,}$/i", $item['thumb1'])) $item['thumb1'] = '';
	if($item['thumb2'] && !preg_match("/^[a-z0-9_\-\.\:\/]{30,}$/i", $item['thumb2'])) $item['thumb2'] = '';
	if($type == 0) {
		$nopic = DT_SKIN.'image/nopic60.gif';
		$imgs[] = $item['thumb'] ? DT_PATH.$item['thumb'] : $nopic;
		$imgs[] = $item['thumb1'] ? DT_PATH.$item['thumb1'] : $nopic;
		$imgs[] = $item['thumb2'] ? DT_PATH.$item['thumb2'] : $nopic;
	} else if($type == 1) {
		$nopic = DT_SKIN.'image/nopic240.gif';
		$imgs[] = $item['thumb'] ? str_replace('.thumb.', '.middle.', DT_PATH.$item['thumb']) : $nopic;
		$imgs[] = $item['thumb1'] ? str_replace('.thumb.', '.middle.', DT_PATH.$item['thumb1']) : $nopic;
		$imgs[] = $item['thumb2'] ? str_replace('.thumb.', '.middle.', DT_PATH.$item['thumb2']) : $nopic;
	}
	return $imgs;
}
