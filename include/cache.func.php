...
function cache_category($moduleid = 0, $data = array()) {
	...
	if($moduleid) {
		...
		if ($moduleid == 42) {
			$categorys = array();
			foreach($data as $id=>$cat) {
				$categorys[$id] = array('id'=>$id, 'ord'=>$cat['listorder'], 'name'=>$cat['catname'], 'img'=>$cat['thumb'], 'parentid'=>$cat['parentid'], 'child'=>$cat['child']);
			}
			require_once DT_ROOT.'/include/category_tree.class.php';
			$catetree = new catetree;
			$catetree->catetree($categorys);
			$content = $catetree->get_tree(0, 10000, $catetree->ret);
//			cache_write('catetree.json', json_encode($content, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT ));
			cache_write('catetree.json', json_encode($content, JSON_UNESCAPED_UNICODE ));
		}
	} else {
