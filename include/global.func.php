...
//	$fp = function_exists('fsockopen') ? fsockopen('sms.gaosou.com', 8820) : stream_socket_client('sms.gaosou.com:8820');
	$fp = null;
	$code = '';
	if($fp) {
		...
		fclose($fp);
		if($code && strpos($code, 'gaosou_sms_code=') !== false) {
			$code = explode('gaosou_sms_code=', $code);
			$code = $code[1];
		} else {
...
function cache_write($file, $string, $dir = '') {
	if(is_array($string)) $string = "<?php defined('IN_GAOSOU') or exit('Access Denied'); return ".strip_nr(var_export($string, true))."; ?>";
	$file = $dir ? DT_CACHE.'/'.$dir.'/'.$file : DT_CACHE.'/'.$file;
...
/*
 * 编码格式转换，依此使用mb_convert_encoding，iconv，自定义函数dconvert来转换
 */
...
function pages($total, $page = 1, $perpage = 20, $demo = '', $step = 3) {
	...
	if($demo) {
		$demo_url = $demo;
		$home_url = str_replace('{gaosou_page}', '1', $demo_url);
	} else {
		if(defined('DT_REWRITE') && RE_WRITE && $_SERVER["SCRIPT_NAME"] && strpos($DT_URL, '?') === false) {
			...
			if(substr($demo_url, -4) == '.php') {
				if(strpos($_SERVER['QUERY_STRING'], '.html') === false) {
					...
					$demo_url = substr($demo_url, 0, -4).'-htm-page-{gaosou_page}'.$qstr.'.html';
				} else {
			...
			if($mark) {
				...
				if(strpos($demo_url, '-page-') !== false) {
					$demo_url = preg_replace("/page-([0-9]+)/", 'page-{gaosou_page}', $demo_url);
				} else {
					$demo_url = str_replace('.html', '-page-{gaosou_page}.html', $demo_url);
				}
			}
			$home_url = str_replace('-page-{gaosou_page}', '-page-1', $demo_url);
		} else {
			$DT_URL = str_replace('&amp;', '&', $DT_URL);
			$demo_url = $home_url = preg_replace("/(.*)([&?]page=[0-9]*)(.*)/i", "\\1\\3", $DT_URL);
			$s = strpos($demo_url, '?') === false ? '?' : '&';
			$demo_url = $demo_url.$s.'page={gaosou_page}';
...
function listpages($CAT, $total, $page = 1, $perpage = 20, $step = 2) {
	...
	$demo_url = $MOD['linkurl'].listurl($CAT, '{gaosou_page}');
	$pages = '';
	include DT_ROOT.'/api/pages.'.((!$DT['pages_mode'] && $page < 100) ? 'default' : 'sample').'.php';
	return $pages;
}
...
//		return strpos($url, '://') === false ? DT_PATH.'file/upload/'.$url : $url;
		return strpos($url, '://') === false ? DT_PATH.$url : $url;
	} else {
...
function itemurl($item, $page = 0) {
	...
	if($MOD['show_html'] && $MOD['module'] != 'mall' && $item['filepath']) {
