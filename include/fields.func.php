...
function fields_show($itemid, $left = '<td class="tl">', $right = '<td>', $values = array(), $fd = array()) {
	...
	if($v['html'] == 'hidden') {
		...
		switch($v['html']) {
			...
			break;
			case 'editor':
				$toolbar = isset($group_editor) ? $group_editor : 'gaosou';
				if(DT_EDITOR == 'fckeditor') {
					...
					$html .= '<textarea name="post_fields['.$v['name'].']" id="'.$v['name'].'" style="display:none">'.$value.'</textarea>'. deditor($moduleid, $v['name'], '', $v['width'], $v['height']).'<span class="f_red" id="d'.$v['name'].'"></span>';
				}
			break;
			case 'area':
