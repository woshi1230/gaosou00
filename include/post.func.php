...
function dstyle($name, $value = '') {
	...
	$style .= '<input type="hidden" name="'.$name.'" id="color_input_'.$gaosou_style_id.'" value="'.$color.'"/><img src="'.DT_PATH.'file/image/color.gif" width="21" height="18" align="absmiddle" id="color_img_'.$gaosou_style_id.'" style="cursor:pointer;background:'.$color.'" onclick="color_show('.$gaosou_style_id.', Dd(\'color_input_'.$gaosou_style_id.'\').value, this);"/>';
	return $style;
}
...
function dcalendar($name, $value = '', $sep = '-') {
	global $gaosou_calendar_id;
	$calendar = '';
	$id = str_replace(array('[', ']'), array('', ''), $name);
	if(!$gaosou_calendar_id) {
		$gaosou_calendar_id = 1;
...
function tpl_select($file = 'index', $module = '', $name = 'template', $title = '', $template = '', $extend = '') {
	...
	$select = '<span id="gaosou_template_'.$gaosou_tpl_id.'"><select name="'.$name.'" '.$extend.'><option value="">'.$title.'</option>';
	...
	$select .= '</select></span>';
	if(defined('DT_ADMIN')) $select .= '&nbsp;&nbsp;<a href="javascript:tpl_edit(\''.$file.'\', \''.$module.'\', '.$gaosou_tpl_id.');" class="t">'.$L['post_edit'].'</a> &nbsp;<a href="javascript:tpl_add(\''.$file.'\', \''.$module.'\');" class="t">'.$L['post_new'].'</a>';
	return $select;
}
...
function imgcat_select($name = 'catid', $title = '', $catid = 0, $userid = 0, $extend = '') {
	global $db;
	if($userid) {
		$data = array();
		$result = $db->query("SELECT * FROM {$db->pre}image_cat WHERE userid='$userid' ORDER BY listorder,catid");
		while($r = $db->fetch_array($result)) {
			$data[$r['catid']] = $r;
		}
		$a = array();
		...
		$tree = new tree;
		$tree->tree($categorys);
		$option = $tree->get_tree(0, "<option value=\\\"\$id\\\">\$spacer\$name</option>").'</select>';
	}
...
function is_uri($url) {
	return preg_match("/^[A-Za-z0-9_\-\/\.\#\&\?\;\,\=\%\:]{4,}$/", $url);
}
...
//	if(!defined('DT_ADMIN') && (!$userid || $userid != match_userid($file))) return false;
	if(!defined('DT_ADMIN') && (!$userid)) return false;
