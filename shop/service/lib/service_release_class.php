...
class service_release_class extends keke_shop_release_class {
	...
	public function pub_service() {
		...
		if($this->_service_config['overdue_type']=='forever'){
			$service_obj->setExist_time();
			$service_obj->setOverdue_type("forever");
		}elseif($this->_service_config['overdue_type']=='custom'){
			$service_obj->setExist_time(strtotime ( $this->_std_obj->_release_info ['txt_exist_day'] ) + $rel_time );
			$service_obj->setOverdue_type("custom");
		}else{
			$service_obj->setExist_time(time()+60*60*24*(intval($this->_service_config['overdue_type'])));
			$service_obj->setOverdue_type($this->_service_config['overdue_type']);
		}
